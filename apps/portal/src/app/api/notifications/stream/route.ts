 ...
 *
 * Mirrors getClientIdentity() in apps/portal/src/app/notifications/actions.ts.
 ...
 *
 * ADR-003: identity resolution is the first step before any subscription.
...
async function resolveClientIdentity(): Promise<{
  ...
  // SECURITY: if the caller is not a CLIENT (e.g. ACCOUNTANT or unauthenticated),
  // this route returns 401. The ACCOUNTANT stream is handled by apps/admin (TASK-016-006).
 ...
 * To subscribe to the correct channel, we resolve the DB User.id from the clerkUserId.
 *
 ...
 *
 * Returns null if the user is not found in the DB (e.g. not yet invited).
 */
