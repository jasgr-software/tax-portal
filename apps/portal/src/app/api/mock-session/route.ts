/**
 * apps/portal/src/app/api/mock-session/route.ts — Mock session fixture endpoint
 *
 * ONLY ACTIVE when AUTH_PROVIDER=mock (e2e + local dev default).
 * Returns 404 in production (AUTH_PROVIDER=clerk or absent).
 *
 * Purpose: e2e test fixtures POST to this endpoint to create a signed
 * __mock_session cookie for a given clerkUserId + role. The middleware
 * trusts this signed cookie (HMAC-SHA256) — the browser cannot forge it.
 *
 ...
 *
 * Security: this endpoint is explicitly NOT served in production
 * (AUTH_PROVIDER=clerk disables it). Do not add it to a real Clerk-configured deploy.
 *
