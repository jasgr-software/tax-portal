/**
 * apps/admin/src/app/messages/_components/MessageComposer.tsx
 *
 * Message compose + attach control for the ACCOUNTANT (admin) per-engagement thread view.
 *
 * Renders a textarea for composing a plain-text message (body) and an optional file picker
 * for attaching one file. On submit: (1) calls sendMessageAction (engagement thread) to
 * persist the message, then (2) if a file is selected, calls attachMessageAction.
 *
 * AC-MSG-001-04: ACCOUNTANT can send messages in the engagement thread.
 * AC-MSG-002-01: message appended with sender + plain-text body (sendMessageAction).
 * AC-MSG-004-01: ACCOUNTANT can attach a file when sending a message (attachMessageAction).
 ...
 * CS-TS-003: mirrors MessageComposer in apps/portal (same UX, per-surface server actions). // CS-TS-003
 * ADR-006: admin surface (ACCOUNTANT-facing); per-surface server actions. // ADR-006
...
/**
 * MessageComposer — compose + send (+ optional attach) for the ACCOUNTANT.
 *
 * AC-MSG-001-04: ACCOUNTANT can send messages.
