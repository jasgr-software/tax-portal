/**
 * apps/admin/src/app/messages/_components/ThreadList.tsx
 *
 * Thread list for the ACCOUNTANT (admin) messages surface.
 *
 * Renders the list of threads visible to the ACCOUNTANT viewer, each decorated with the
 ...
 * AC-MSG-001-04: ACCOUNTANT can open any thread and contribute.
 *
 * CS-TS-003: mirrors ThreadList in apps/portal (same structure). // CS-TS-003
 * ADR-006: admin surface (ACCOUNTANT-facing) — StartGeneralThread affordance is in the parent page. // ADR-006
...
export interface ThreadListProps {
  /** Threads visible to the ACCOUNTANT viewer (RLS-scoped, server-fetched). */
...
/**
 * ThreadList — renders the ACCOUNTANT's visible threads with per-viewer unread indicators.
 *
 * AC-MSG-005-01/-02: each thread row shows an unread badge if hasUnread=true.
 * AC-MSG-001-04: each thread is a link — the ACCOUNTANT clicks to read + contribute.
 *
...
export function ThreadList({ threads }: ThreadListProps) {
  if (threads.length === 0) {
    return (
      ...
      >
        No message threads yet. Start a new direct thread above.
      </div>
    );
  }
...
/**
 * ThreadListItem — one thread row in the ACCOUNTANT's thread list.
 *
