/**
 * apps/admin/src/app/engagements/[engagementId]/messages/actions.ts
 *
 * Server actions for the admin (ACCOUNTANT) per-engagement messaging surface.
 *
 * TASK-017-003: Send-message + mark-thread-read for the ACCOUNTANT on the per-engagement thread.
 * TASK-017-004: Attach (on send) + requestAttachmentUrlAction (ACCOUNTANT guard) — scan-before-available.
 *
 * Acceptance criteria:
 *   AC-MSG-001-02 — thread visible only to participants (identity guard + request pool).
 *   AC-MSG-001-04 — both parties can send messages (ACCOUNTANT surface: sendMessageAction).
 ...
 *   AC-MSG-014-01 — when the ACCOUNTANT sends, the CLIENT(s) receive a new-message notification
 *                   (emitted inside appendMessage post-write — AC-MSG-014-01).
 *
 ...
 *   Non-ACCOUNTANT or unauthenticated callers are refused before any DB write.
 *
 ...
 *   An ACCOUNTANT participant of thread A CANNOT bypass the RLS gate.
 *
 * ADR-003: server-side authority — getAccountantIdentity() before every DB access. // ADR-003
 * ADR-006: admin surface (ACCOUNTANT-facing); portal surface is the CLIENT mirror. // ADR-006
 ...
 * CS-TS-003: send + mark-read + attach + retrieve mirror exists in apps/portal. // CS-TS-003
...
/**
 * Send a plain-text message in an engagement thread (ACCOUNTANT surface).
 *
 * AC-MSG-001-04: Both parties can send messages in the engagement thread.
 *   This action handles the ACCOUNTANT side; the CLIENT mirror is in apps/portal.
 * AC-MSG-014-01: After the message is persisted, appendMessage emits a new-message
 *   notification to the CLIENT(s). The ACCOUNTANT sender is NEVER self-notified.
 *
 * Flow:
 *   1. getAccountantIdentity() — must be ACCOUNTANT (ADR-003, CS-TS-004). Refused otherwise.
 *   2. Input validation — engagementId and body must be non-empty.
 ...
 *      the CLIENT(s). Sender (ACCOUNTANT) is not self-notified. // AC-MSG-014-01
 *
 * ADR-003: getAccountantIdentity() before any DB access. // ADR-003
 * ADR-006: admin surface (ACCOUNTANT); CLIENT mirror in apps/portal. // ADR-006
...
export async function sendMessageAction(
  ...
  try {
    ...
    console.error("[admin:engagement:sendMessageAction] getOrCreateEngagementThread failed:", err);
  ...
  // Post-write: appendMessage emits new-message notification to CLIENT(s). // AC-MSG-014-01
  ...
  try {
    ...
    console.error("[admin:engagement:sendMessageAction] appendMessage failed:", err);
...
/**
 * Mark the engagement thread as read for the current ACCOUNTANT viewer.
 *
 ...
 *   4. withRequestContext(ACCOUNTANT) → markThreadRead(threadId) — request pool upsert.
 *
 * ADR-003: getAccountantIdentity() before any DB access. // ADR-003
 * ADR-005: sec.pol_ThreadReadState BLOCK enforces own-row write. // ADR-005
 * ADR-006: admin surface (ACCOUNTANT); mirror in apps/portal. // ADR-006
...
export async function markThreadReadAction(
  ...
  try {
    ...
    // CS-GEN-001: server-level log only. // CS-GEN-001
    console.error("[admin:engagement:markThreadReadAction] getOrCreateEngagementThread failed:", err);
    return { success: false, error: "Unable to resolve the message thread. Please try again." };
  }
...
/**
 * Attach a file to an existing message (ACCOUNTANT surface).
 *
 ...
 * CS-TS-003: mirror in apps/portal engagement messages actions. // CS-TS-003
 *
...
export async function attachMessageAction(
  ...
  try {
    ...
    console.error("[admin:engagement:attachMessageAction] storeAndScanAttachment failed:", err);
...
/**
 * Request a short-lived participant-scoped signed URL for a message attachment (ACCOUNTANT surface).
 *
 ...
 *   The storage key is resolved server-side under RLS (sec.pol_MessageAttachment FILTER).
 *   The ACCOUNTANT surface uses the ADMIN POOL for reading (full visibility), but
 *   authorizeThenSignAttachment still uses the request pool for RLS-governed resolution
 *   because the attachment itself may belong to any thread. The ACCOUNTANT is always a
 *   participant (FILTER predicate admits IS_MEMBER('app_admin_role')=1).
 *
 * Flow:
 *   1. getAccountantIdentity() — must be ACCOUNTANT (ADR-003, CS-TS-004). Refused otherwise.
 ...
 *      Gate: attachment resolved by RLS + status='active'. Sign the server-resolved key.
 *
 * ADR-003: getAccountantIdentity() before any DB access. // ADR-003
...
export async function requestAttachmentUrlAction(
  ...
  // withRequestContext sets SESSION_CONTEXT. ACCOUNTANT is IS_MEMBER('app_admin_role')=1
  // → FILTER predicate admits ACCOUNTANT to all attachments. Still goes through request
  // pool for consistent authorize-before-mint discipline (ADR-009).
  // CS-TS-001: authorizeThenSignAttachment under withRequestContext. // CS-TS-001
  // CS-GEN-001: signed URL NEVER logged; audit targetId = attachmentId only. // CS-GEN-001
  const result = await withRequestContext(
    identity.clerkUserId, // CS-TS-004: from verified session
