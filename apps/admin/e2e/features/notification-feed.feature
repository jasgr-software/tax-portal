# apps/admin/e2e/features/notification-feed.feature
#
# Gherkin acceptance scenarios — Accountant notification feed (EPIC-016 / TASK-016-007)
#
# Acceptance format: gherkin (prose-bound)
# Executable proof: apps/admin/e2e/specs/notification-feed.spec.ts
#
...
Feature: Accountant notification feed (admin surface)
  In order to stay informed about client document uploads and engagement activity
  ...
  # ─── Source-event: document upload → accountant notification ─────────────────

  # AC-MSG-013-03
  @AC-MSG-013-03
  Scenario: Accountant receives in-portal notification when client uploads a document
  ...
  # ─── Real-time notification arrival (both surfaces) ───────────────────────────

  # AC-MSG-012-01
  @AC-MSG-012-01
  Scenario: Notification surfaces without manual refresh (accountant)
  ...
  # AC-MSG-012-02
  @AC-MSG-012-02
  Scenario: Multiple events appear in real time (accountant)
  ...
  # AC-MSG-012-03
  @AC-MSG-012-03
  Scenario: Unread badge increments without manual refresh (accountant)
  ...
  @AC-MSG-015-02
  Scenario: Viewing the linked item marks the notification read (accountant)
  ...
  # AC-MSG-015-03
  @AC-MSG-015-03
  Scenario: Read state reflected; no separate dismiss action (accountant)
  ...
  # ─── Unread-count badge (both surfaces) ──────────────────────────────────────

  # AC-MSG-017-01
  @AC-MSG-017-01
  Scenario: Unread badge is present in the navigation on any admin area
  ...
  # AC-MSG-017-02
  @AC-MSG-017-02
  Scenario: Badge shows the correct unread count (accountant)
  ...
  # AC-MSG-017-03
  @AC-MSG-017-03
  Scenario: Badge updates when notifications are marked read or a new one arrives (accountant)
  ...
  # ─── Source-event: client actions → client notifications (admin surface confirms) ─
  ...
  # ─── In-portal feed as the authoritative record ───────────────────────────────
