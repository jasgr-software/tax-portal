...
- **TENET-008 — Stay cloud-neutral; bias every choice so Azure is the cheap default.** The production platform is deferred (ADR-007). Until it is chosen, we **target a capability contract, default every implementation to the Azure-cheapest option that satisfies it, and keep each cloud touchpoint behind a swappable port.** For this bursty, idle-most-of-the-day workload the cheapest Azure options (scale-to-zero compute, auto-pause DB, tiered blob storage) are also the least proprietary — so "cheap Azure" and "low lock-in" are the same target, not a trade-off.

   No proprietary runtime or platform SDK reaches app code: no edge runtime, no `@vercel/*`, no Azure Functions programming model, no Azure SignalR/Web PubSub, no cloud secrets SDK (secrets arrive via env, 12-factor), no Azure Communication Services. Storage, signed-URL minting, email, and realtime transport are reached through interfaces, never a provider SDK in a route handler. Database schema stays in the **box SQL Server 2022 ∩ Azure SQL Database** intersection (RLS policies, `SESSION_CONTEXT`, temporal tables, filtered indexes are in; cross-database queries, SQL Agent, CLR, FILESTREAM, linked servers are out). See `decisions/ADR-013-cloud-portability-azure-readiness.md` for the per-concern resolution table and the Azure-cheapest default targets. This neither picks a platform nor adds an Azure dependency — the bias lives in what we avoid.
