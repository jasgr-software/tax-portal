...
5. **Gate Authoring Rules enforcement** — enumerate each new required gate introduced in the epic by scanning `docs/tasks/done/TASK-EEE-*` and `docs/tasks/done/BUG-EEE-*` (where `EEE` is the current epic number) for `**Introduces-gate:** yes` in their task-spec headers. For each such task, open the Work Log and verify all three evidence items are present per `.claude/agent-stack.md` § Gate Authoring Rules: (a) run URL + specific job/step name, (b) named code path, (c) counterfactual. Tasks with `**Introduces-gate:** advisory` do not require evidence but should be listed in the epic-close writeup for visibility. Your epic-close writeup must name each required gate introduced and cite the Work Log entries that satisfy each of the three evidence items — a summary assertion such as "all gates have evidence" is insufficient. If any required gate lacks complete evidence, **STOP and reject** epic close and report the gaps to the SA. The SDET should have caught these at task review; an epic-close miss means the gate-authoring review was incomplete.
6. **Choose validation mode based on epic output:**
...
7. **Reject** if any user workflow is incomplete or broken (code epics), or if any artifact is incomplete or fails to satisfy its acceptance criteria (document epics) — be critical, not lenient
8. If approved, update the SRS to mark requirements as `Implemented` and archive the epic file
