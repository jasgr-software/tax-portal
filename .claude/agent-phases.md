...
| **Review**         | Spawn SDET for each `review` task. Handle rejections. **Architecture scan** after all tasks pass — dispatch the Architecture Agent (`.claude/agents/architect.md`) against the integrated diff; it returns a deviation report (drift from ADRs/tenets/C4/strategy) that the SA dispositions as blocking/non-blocking. Fix forward if violations found. Update PROGRESS.md.                                                                                                                                                                                                                      |
...
| **Close-prep**     | Update architecture model + create/supersede ADRs — dispatch the Architecture Agent (`.claude/agents/architect.md`) to update the C4 model and author/supersede ADRs under `.architecture/` for decisions the epic established. **Consistency gate.** Archive task/bug/plan files. Verify RA archival. **Retrospective** (Overwatch). **Retro findings are advisory by default** — only concrete gate failures get classified per § Retro Finding Classification. Update PROGRESS.md → move to `## Awaiting PR merge`. Request PR approval. **SA ends invocation.** |
...
| **Review**         | Every `TASK-EEE-*` / `BUG-EEE-*` **for the current epic** has `Status: done` with the `## SDET Review` section completed by the SDET agent (approval breadcrumb in Work Log naming the SDET invocation), every Quality Gates Mandatory box ticked, and `Complexity-actual` + `Completed-at` populated per `agent-stack.md` § Task Metadata Contract. Architecture scan complete — the Architecture Agent's deviation report recorded as a PROGRESS.md session entry. Every arch-scan finding **the SA classified as blocking** has a dispatched fix task that reached `done`.                                                                                                                                                                            |
...
| **Close-prep**     | `RETRO-EEE.md` written. C4 model updated where epic scope changed it and ADRs created/updated under `.architecture/` (via the Architecture Agent) per § ADR Lifecycle. Task files archived to `docs/tasks/done/`. Epic requirement file archived by the RA. Epic moved from `## Current initiative` to `## Awaiting PR merge` in PROGRESS.md. PR raised and PR URL recorded in `## Awaiting PR merge`. (SA then ends invocation; PR merge is the user-in-loop checkpoint per `agent-stack.md` § Autonomy Ceiling.)                                                                                                                                                                                                                                                         |
