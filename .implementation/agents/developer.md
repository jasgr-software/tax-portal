...
4. **Read the brief's acceptance criteria.** For every AC ID in the task spec's `**Acceptance criteria:**` field, understand the observable behavior it asserts — these are what your executable tests must verify. If the task spec has no `**Acceptance criteria:**` field, **stop and escalate to the IO** — Plan was incomplete.
...
8. If all developer-owned gates pass, set status to `review`, set `Complexity-actual` to your 1–5 rating of the actual effort (per `.implementation/ENGINE.md` § Task Metadata Contract), and update Work Log with results — **when the brief mandates e2e, include actual test execution output (pass/fail counts, test names) in the Work Log as proof of execution**. The SDET will reject the task if `Complexity-actual` is empty or not in `1`–`5`.
