...
1. Set `status` to `in-progress` in the front matter, set `started_at` to current UTC ISO 8601 (e.g., `date -u +%Y-%m-%dT%H:%M:%SZ`), set `complexity_estimate` to your honest 1–5 rating **before reading implementation notes** (1=very easy, 5=very hard — inflating to match actual defeats the metric), update `updated_by` and **Work Log**. All four edits in the same Edit call. See `.implementation/ENGINE.md` § Task Metadata Contract.
...
8. If all developer-owned gates pass, set `status` to `review`, set `complexity_actual` to your 1–5 rating of the actual effort (per `.implementation/ENGINE.md` § Task Metadata Contract), and update Work Log with results — **when the brief mandates e2e, include actual test execution output (pass/fail counts, test names) in the Work Log as proof of execution**. The SDET will reject the task if `complexity_actual` is empty or not in `1`–`5`.
