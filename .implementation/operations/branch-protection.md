...
Branch protection is predicate (b) of the three-part graduation requirement for promoting PR-merge to autonomous. See `.implementation/ENGINE.md` § Autonomy Ceiling item 3:
...
Decision reference: decision #1A from `.implementation/tasks/PROGRESS.md` `## Current initiative` planning entry.
...
| `required_pull_request_reviews` | `null` | Solo developer cannot approve their own PR (GitHub mechanically blocks self-approval). Required CI is the substantive gate; quad review via workflow rules covers second-pair-of-eyes for `.implementation/agents/*.md` and `.implementation/ENGINE.md` PRs |
...
| `required_linear_history` | `false` | Squash merge produces linear history naturally; hard-enforcing this would block merge commits needed for hotfix mini-epics per `.implementation/PHASES.md` § Post-Close Protocol |
...
Use the disable command only when branch protection is blocking a legitimate hotfix flow before the item-3 PR-merge promotion (`.implementation/ENGINE.md` § Autonomy Ceiling item 3) lands. This is rare — the normal path is to open a PR from a hotfix branch and let CI gate it. Disable only when CI itself is broken and the fix cannot land any other way.

**Required before disabling:** document the reason in `.implementation/tasks/PROGRESS.md` (append a session entry with the specific blocker). Re-enable branch protection immediately after the hotfix merges.
...
- `.implementation/ENGINE.md` § Autonomy Ceiling item 3 — the graduation predicate that cites branch protection as predicate (b); includes the full three-predicate list for PR-merge auto-on-green promotion
- `.implementation/tasks/done/TASK-LOE-001-ci-workflow.md` — the task that landed `.github/workflows/ci.yml` with the job names this runbook references
- `.implementation/tasks/done/TASK-LOE-003-validate-gates-script.md` — the task that landed `scripts/validate-gates.sh`, the independent programmatic backstop (predicate (c) in item 3)
- `.implementation/tasks/PROGRESS.md` § Current initiative — the planning entry containing decision #1A that this runbook implements
