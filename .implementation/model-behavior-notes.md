...
> **Status: seeded (2026-04-28).** Initial four entries seeded from the chore PR #8 quad review via the Lens B mechanism (`.implementation/ENGINE.md` § Main Session Rules). Each entry reflects a failure mode originally observed in the source project (journey-for-jasmine) for which a mitigation rule now ships in this project's `.implementation/ENGINE.md` (introduced via PR #5 / PR #8, 2026-04-26 / 2026-04-27). The chore is not a feature epic; none of the four mitigations has yet been stress-tested in this project's feature work. Entries are expected to be revised, retired, or expanded as Epic 001 and subsequent feature epics either confirm recurrence or demonstrate the mitigation has retired the failure mode.

## Purpose

This file is the reference for **Lens B of quad review** — the model-behavior lens. Reviewers (IO, SDET, Overwatch) consult this file when evaluating workflow-file changes to decide whether any rule being modified is load-bearing against a known failure mode of the primary model (currently Claude Opus / Sonnet as configured per-role in `CLAUDE.md`'s Agent Team table).
...
- **Mitigation currently in place:** `.implementation/ENGINE.md` § Stuck-Loop Killswitch — 3 consecutive identical-failure-mode attempts → halt, create BUG file, set task status `needs-user-direction`, fire `PushNotification`.
...
- **Mitigation currently in place:** `.implementation/ENGINE.md` § Tool Hygiene / PushNotification — three-event allowlist (Docker pre-flight escalation, credential-pattern hit on `git add`, Stuck-Loop Killswitch firing) with explicit prohibitions on routine-status fires and a spam-loop guard against re-fires for the same condition.
...
- **Mitigation currently in place:** `.implementation/ENGINE.md` § Gate Authoring Rules — Work Log evidence triad (run URL + named job/step, named code path the gate exercises, counterfactual: "with the gate disabled the failure reproduces"). Tasks declare `Introduces-gate: yes` to surface the requirement explicitly. SDET rejects at review if any item is missing.
...
### `clarif-deflection` — model deflects routine ambiguity to user instead of making a bounded slice-level decision

- **Observed when:** IO during Plan/Clarify, or a developer mid-task, when a slice-level ambiguity surfaces. Pattern: model identifies an ambiguity, packages it as a question for the user, pauses for user input — even when the ambiguity is routine UX/copy/wording the IO has authority to resolve against the brief's acceptance criteria.
- **Symptom:** Agent halts on every ambiguity and surfaces it to the user. Workflows that should run autonomously stall on routine decisions. User receives a steady stream of "should this button say X or Y?" type questions that the IO could resolve by picking the most-consistent option and documenting the reasoning.
...
- **Mitigation currently in place:** `.implementation/ENGINE.md` § Acceptance & Methodology (the IO clarifies slice-level ambiguity against the brief's acceptance criteria) + § Autonomy Ceiling item 6 (the IO escalates to the brief author **only** when criteria are missing/untestable/contradictory, and *raises* genuinely-architectural ambiguity upstream via `OPEN-QUESTIONS.md` — it does not punt routine slice decisions to the user). The brief's acceptance criteria are the resolution authority; the IO picks the most-consistent reading and records it.
- **Counterfactual:** Without the explicit "IO resolves slice ambiguity, doesn't punt" rule, every ambiguity becomes a user-pause point. Lights-out execution is impossible — the user becomes the bottleneck on routine decisions.
- **First observed:** Source project (journey-for-jasmine), pre-port. Originally mitigated via the RA's SRS-resolution autonomy; re-homed to the IO's slice-level clarification when the team was refactored into `.implementation/` (2026-06-14). Not yet stress-tested in feature work on this project.
- **Last cited:** 2026-04-28 (chore PR #8 quad review, under the prior RA framing).
