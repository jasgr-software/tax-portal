...
**Start:** Re-invoked for Close-finalize — PR #40 squash-merged to `main` @ `70ea10e` (2026-06-16). Gate 8 (post-merge CI) confirmed GREEN by the main session (`CI` ✅ + `CodeQL` ✅ via `gh run list --branch main` on `70ea10e`); gate 9 N/A (`Brief-deploys: no`, ADR-007). Driving the final close.
**Phase-transition reflex (Close-prep→Close-finalize):** swept the IO Close-prep session entry (incl. the "PR OPENED — PR #40" note) to this archive under the Close-finalize marker; updated `## Current initiative` to _None active_ + EPIC-002 delivered + next-ready EPIC-003; emptied `## Awaiting PR merge`; appended the Close-finalize session entry to the live PROGRESS.md.
...
**9-gate scorecard (at Close-prep):** (1) per-task submission 5/5 ✅ · (2) SDET Review 5/5 ✅ · (3) Overwatch Audit ✅ (0 blocking) · (4) IO Design scan ✅ · (5) Container Smoke ✅ CONDITIONAL-PASS (UI PASS / Infra cond-pass; `sqlserver (unhealthy)` healthcheck SA mismatch documented; clean-volume bootstrap env-blocked → CI-as-gate substitution carried from EPIC-004, user-accepted) · (6) SDET Acceptance-validation ✅ · (7) SDET CI gate ✅ · (8) Post-merge CI — Close-finalize · (9) staging smoke — N/A (`Brief-deploys: no`).
...
**PR OPENED (main session, 2026-06-16):** **PR #40** — https://github.com/jasgr-software/tax-portal/pull/40 — base `main` ← `brief-002-services-catalog-management` (10 commits `c500053`→`33826d4`: 5 tasks + 4 bugs + Close-prep ledger). **Application-code PR → reviewed lane** (`.orchestration/MERGE-POLICY.md` Lane B: panel → fix → resolve threads → merge on green required CI, no protection toggle). Next: Conductor Review (`/pr-review 40`).

<!-- Swept at Dispatch→Review (TASK-002-001) transition (2026-06-16) -->

### IO Plan — BRIEF-002 — 2026-06-16
**Start:** Engine idle (`## Awaiting PR merge` empty; no active slice; BRIEF-004 delivered). Slice-start gate CLEAR. Invoked to drive BRIEF-002 (services-catalog management). Ran Plan: Ingest → Clarify → Design → Decompose.
**Phase-transition reflex (idle → Plan):** swept the BRIEF-004 Close-prep session entry to `PROGRESS-ARCHIVE.md` (idle→BRIEF-002 marker); repopulated `## Current initiative` with the BRIEF-002 slice + 5-task table + AC map; appended this entry. Backlog triage: `## Awaiting PR merge` empty; `## Active bugs` none; `## Open retro action items` all carried with explicit reasons (infra DB-bootstrap + env RATE_LIMIT items are directly relevant to this slice's Smoke/e2e — kept live) → gate PASS.
**Actions:**
- **Ingest.** Read BRIEF-002 in full + grounded against the live codebase: `db/policies/0002-service-readable.sql` (confirmed the latent gap — the FILTER and all four BLOCK predicates share `sec.fn_service_access`, whose branch 3 grants CLIENT → a CLIENT principal currently PASSES the write block predicate), `prisma/schema.prisma` (Service entity present, right shape), `packages/db` (service repo, `client.ts` `$extends` request wrapper, `withRequestContext`/`withClerkIdentity`, `engagement-request.rls.test.ts` tier-3 pattern), `apps/admin` (scaffolded; middleware role gate via `applyAdminAuth`; `getIdentity → withRequestContext` hand-off in `page.tsx`; `mock-session` route + `e2e/fixtures/auth.ts` `setupAccountantSession`). Read cited upstream refs that are present (ADRs via the brief). Confirmed reuse-don't-rescaffold.
- **Clarify.** All 7 ACs are observable + testable. Methodology recorded: gherkin (7 scenarios → `.feature` mirror), e2e required (apps/admin, mocked provider), tier-3 write-boundary + persistence, cross-surface loop, AC-id test-tag contract, container smoke before Validate. No untestable/contradictory criteria → no escalation to brief author.
- **Docker pre-flight:** `docker info` exit 0, Server Version 29.4.1 — PASS.
- **Design.** The write/read authorization diverges for CLIENT (read yes, write no), so the fix is a **separate write-predicate function** `sec.fn_service_write_access` (admin OR ACCOUNTANT only; no CLIENT branch; null→fail-closed) wired to the four BLOCK predicates, while the FILTER predicate keeps `fn_service_access` (CLIENT reads active). Writes flow accountant action → server action (`getIdentity` → `withRequestContext`) → request-scoped `db` wrapper (SESSION_CONTEXT) → Service entity, authorized at the policy. This realizes ADR-005's existing mandate → **no upstream/architectural question raised**; recorded as a `// DECISION:` to be carried in TASK-002-001.
- **Local design-coherence check — PASS.** All 7 ACs traced to ≥1 tagged test at the prescribed tier; primary risk isolated as the dependency-root task with `Introduces-gate: yes`; ADR-006/-003/-002/-004/-005/-012 + REQ-DOOR-002 constraints each owned by a task's SDET focus; out-of-scope (AC-DOOR-002-04, templates, inbox, hard-delete, 2FA) respected; apps/admin single-surface exception (no portal mirror) documented in TASK-002-003 so it does not trip the cross-surface-parity rule.
- **Decompose.** Created 5 task files (TASK-002-001…005), all `Status: backlog` with `Impl`, `E2e-required`, `**Acceptance criteria:**`, `**Upstream refs:**`, `**Introduces-gate:**`, `Brief-type`/`Brief-deploys` mirrored, dependency chain 001→002→003→004→005.
- **Branch:** `brief-002-services-catalog-management` created from `main`.
**End:** Plan exit condition met. Entered **Dispatch**. First dispatch: TASK-002-001 (write-boundary policy + tier-3 RLS test).
...
**End:** Close-prep exit condition met. BRIEF-004 subsequently merged (PR #38 @ `0444551`) and Close-finalize completed (gate 8 post-merge CI green). Slice DELIVERED 2026-06-16.
...
### IO Close-finalize — BRIEF-004 (EPIC-004) DELIVERED — 2026-06-16
**Trigger:** Conductor merged PR #38 (squash → `main` @ `0444551`, 2026-06-16) after the `/pr-review` panel → `/pr-fix` → thread-resolution cycle. Re-invoked for Close-finalize (gates 8–9).
**Gate 8 — Post-merge CI: PASS.** Post-merge `main` @ `0444551`: workflow `CI` ✅ + `Code Quality: Push on main` ✅ (required `lint-and-typecheck` + `security-scan` green, plus `test-admin` + `test-portal` green — the latter fixed by the `/pr-fix` "Build workspace packages" step added to the test jobs, closing the carried test-portal CI-job design gap).
**Gate 9 — Post-merge staging smoke: N/A** (`Brief-deploys: no`, ADR-007).
**Final 9-gate scorecard:** 1 PASS · 2 PASS · 3 PASS (0 blocking) · 4 PASS · 5 env-blocked (user-accepted CI substitution) · 6 PASS (11/11 AC) · 7 PASS · 8 PASS · 9 N/A.
**Panel outcome (gates beyond the engine's 9):** 3-lens advisory request-changes (1 blocker + 8 major + 4 minor → 13 deduped) all dispositioned — genuine bugs/security FIXED (fail-closed auth, Secure cookie, admin-page role re-check, narrowed `/api/*` exemption, trusted-proxy rate-limit key, sign-up rate-limit, fail-closed middleware); 3 over-eng findings dispositioned-with-rationale as the intentional deferred-Clerk seam (threads resolved). Fixer commits `c89689d`, `01b4219`.
**Conductor write-back (Validate):** Planning agent flipped the 11 in-scope EPIC-004 AC `planned`→`verified` in COVERAGE (verified total 13→24); EPIC-004 rolled `planned`→`delivered` in ROADMAP; EPIC-002/EPIC-003 dependencies satisfied (now unblocked); 4 deferred 2FA AC unchanged. Evidence basis [A] = CI (user-accepted substitution for env-blocked container smoke).
**Carried follow-ups (HANDOFF-004):** (1) host-side local-DB bootstrap + Prisma-tooling fix (clean-volume DB/login creation; Prisma port-in-authority; `!`-free logins; migrate-deploy P3019); (2) `.env.example` RATE_LIMIT vars (permission-walled — user action); (3) 2FA-enablement slice swaps the mock for real Clerk test-mode + re-validates the 4 deferred AC + the now-mocked AUTH-006/009/010 against the live provider; (4) admin `page.tsx` switch to request-pool `db` client when it gains real engagement-data queries.
**End:** EPIC-004 delivered. Slice ledger removed from `## Awaiting PR merge` in the live PROGRESS.md; `## Current initiative` records the delivery + next-ready slices (EPIC-002/003). Remaining Conductor steps: docs-lane PR (demo gallery) → Report.

<!-- Swept at Dispatch→Audit transition (2026-06-16) — BRIEF-002 all dispatch nodes done -->

### IO Dispatch (TASK-002-005 — final node) — BRIEF-002 — 2026-06-16
**Start:** Re-invoked after main session committed TASK-002-004 (`done`, `a7e0ab0`; SDET APPROVED, independent re-exec 3 × 17/17 = 51 total, zero flakes, all 6 ACs traced, cross-surface honest/deterministic). All of -001/-002/-003/-004 + BUG-002-001/-002/-003 are `done` & committed. Composing the final dispatch: TASK-002-005 — the `@demo` Playwright walkthrough capturing jane-accountant's catalog-management happy path (list → add → edit → deactivate, inactive state shown) as an AC-tagged screenshot gallery → `docs/demos/EPIC-002/`. Non-gating per DEMO-POLICY (the TASK-002-004 e2e gate is the gate); `@demo` is excluded from `e2e:run` via `--grep-invert @demo` and runs only via `e2e:demo`.
**Phase-transition reflex:** stayed within Dispatch (no phase boundary). Updated `## Current initiative` Phase line + the task table (TASK-002-005 `backlog` → `in-progress (dispatched)`; TASK-002-004 already `done`/`a7e0ab0`); appended this entry. (Prior session entries roll to PROGRESS-ARCHIVE.md at the next phase boundary, not mid-phase.)
**Actions:**
- **Read the reference surface to make the dispatch self-contained:** `.orchestration/DEMO-POLICY.md` (artifact shape, `@demo`/`e2e:demo` exclusion, lifecycle seam), the task spec `TASK-002-005-demo-walkthrough-spec.md`, the model spec `apps/admin/e2e/demo/identity-spine.demo.spec.ts` (DEMO_DIR `path.resolve(__dirname, "../../../../docs/demos/EPIC-NNN")`, `shot()` helper, `setupAccountantSession(page, request)` + `clearSession`, ASSERT-then-`page.screenshot({path, fullPage:true})` pattern, AC-tagged test titles), `apps/admin/e2e/fixtures/auth.ts` (mock-session fixtures), `apps/admin/e2e/specs/services-catalog.spec.ts` (the exact, proven selectors: `Services Catalog` heading, `Add new service` button, `Add service form`, `Service Name`/`Description` labels, `Add Service` submit, `Edit <name>`, `Edit service form`, `Save Changes`, `Deactivate <name>` + confirm-dialog auto-accept, `inactive-badge` testid, `Service: <name> (inactive)` row regex, `uniqueName()` helper), `apps/admin/package.json` (`e2e:demo` = `playwright test --grep @demo`), and the EPIC-004 gallery + DEMO.md convention.
- **Surfaced the capture-integrity hazard explicitly (EPIC-004 lesson):** EPIC-004 shipped a 06≡08 byte-identical stale-capture artifact (confirmed on disk: `05`/`08` both 48626 bytes, `06`/`08` both 109867 bytes — distinct specs/personas/routes yet identical bytes). The dispatch HARD-requires: capture against the freshly-running admin container (no stale image), use `uniqueName()`-style distinct service names per state, and AFTER capture verify the four PNGs are NOT byte-identical across distinct states (each state visibly differs: empty-or-list → add-form → edited-row → inactive-badge row). Do not repeat the EPIC-004 duplicate.
- **Composed the webapp-developer dispatch** with: ENGINE.md + developer.md reads, role tag, the full task spec ref + the (none — non-gating, justified) ACs + upstream refs, the exact gallery-naming convention (`NN-<AC-ID>-<slug>.png`, EPIC-002 owns its own 01–04 sequence), the model-spec + fixture + selector references, the `e2e:demo`/`--grep @demo` run command with `ADMIN_BASE_URL=http://localhost:13001` + `ADMIN_PORT=13001`, the do-NOT-`down -v` / do-NOT-rebuild caveats, the capture-integrity (no-duplicate-bytes) HARD check, the DEMO.md scaffold option, the Docker pre-flight + no-fabrication rule (real execution required even though non-gating), the Dispatch Checkpoint atomic-edit rule, and the strict boundary (edit only `apps/admin/e2e/**` + `docs/demos/EPIC-002/**` + this task file; NO git ops).
**End:** TASK-002-005 dispatched (one dispatch this turn — the final dispatch node). On developer `review`: dispatch the SDET review (non-gating @demo capture verification — gallery landed under `docs/demos/EPIC-002/`, AC-tagged, no byte-identical duplicates across distinct states, mirrors the EPIC-004 convention, `@demo`-excluded from the gating run; atomic close-edit). On SDET approval → main-session commit of the `@demo` spec to `brief-002-services-catalog-management` (app code, slice PR; the generated gallery PNGs + DEMO.md are docs and ship via the docs fast-lane at close), then enter **Review** (design scan of the integrated diff). Dispatch prompt in the `## Next Dispatch` block.

---

### IO Dispatch (SDET review of TASK-002-005 — final node) — BRIEF-002 — 2026-06-16
**Start:** Re-invoked with TASK-002-005 returned at `review`. Developer delivered the final dispatch node: `apps/admin/e2e/demo/services-catalog.demo.spec.ts` (4 `@demo` tests: list / add / edit / deactivate) capturing 4 AC-tagged PNGs into `docs/demos/EPIC-002/` (`01-AC-DASH-010-01-services-catalog-list.png`, `02-AC-DOOR-002-01-add-service.png`, `03-AC-DOOR-002-02-edit-service.png`, `04-AC-DOOR-002-03-deactivate-service-inactive.png`) + scaffolded `DEMO.md` (persona `jane-accountant` + flow `flow-engagement-request`, one `## NN [AC-ID]` section per shot). Gate green (lint / type-check / admin build); `ADMIN_BASE_URL=http://localhost:13001 ADMIN_PORT=13001 e2e:demo` → 4/4 catalog tests PASS; **capture-integrity already verified by the developer — 4 distinct sha256 hashes + 4 distinct file sizes** (no byte-identical dupes; the EPIC-004 06≡08 stale-capture lesson applied). Out-of-scope pre-existing failure noted: EPIC-004 `identity-spine.demo.spec.ts` test-07 (ADMIN_PORT 13001-vs-3001 redirect mismatch, already documented in EPIC-004's DEMO.md caveat). Composing the SDET review dispatch (non-gating @demo capture verification per DEMO-POLICY).
**Phase-transition reflex:** stayed within Dispatch (no phase boundary — last dispatch node still in flight at `review`). Updated `## Current initiative` Phase line + the TASK-002-005 task-table row (`in-progress (dispatched)` → `review (SDET dispatched)` with the 4-PNG / 4-distinct-hash / gate-green note); appended this entry. (Prior session entries roll to PROGRESS-ARCHIVE.md at the next phase boundary — the Dispatch→Audit transition once -005 closes — not mid-phase.)
**Actions:**
- **IO pre-review read of TASK-002-005 + Work Log + spec.** Confirmed task-file/brief consistency: status `review`, `Complexity-actual: 2` (in range), `Started-at`/`Completed-at` set, Dispatch Checkpoint "Starting implementation" Work Log entry present (2026-06-16T16:37:57Z), required spec fields present (`**Acceptance criteria:** none` with the non-gating justification recorded, `**Upstream refs:**` planning EPIC-002 + ADR-006, `**Introduces-gate: no**`), all dev-owned Quality Gate boxes ticked (Security review N/A — capture only), `SDET Review` box open (correct).
- **Surfaced the load-bearing review hazard (EPIC-004 lesson) for the SDET:** the EPIC-004 demo shipped a byte-identical stale-capture pair (06≡08). The developer recorded 4 distinct sha256 hashes + 4 distinct sizes, but per the SDET's own verification discipline the SDET INDEPENDENTLY re-runs `sha256sum docs/demos/EPIC-002/*.png` (confirm 4 distinct) AND spot-checks each PNG shows a visibly distinct, correct state (list / add-form / edited-row / inactive-badge) — not merely distinct bytes by accident. The dispatch also requires confirming `@demo` does NOT leak into the gating `e2e:run` (`--grep-invert @demo`) so its non-gating status is structural, not just asserted.
- **Composed the SDET review dispatch** for TASK-002-005 with: ENGINE.md + sdet.md reads, role tag, the full task spec ref + the (none — non-gating, justified) ACs + upstream refs, the DEMO-POLICY non-gating framing, the HARD capture-integrity check (independent `sha256sum` 4-distinct + per-shot visible-distinct-state spot-check; no stale/duplicate captures), the AC-tag + screen-visible-before-screenshot check, the `@demo`-excluded-from-`e2e:run` structural confirmation, the optional `e2e:demo` re-run (`ADMIN_BASE_URL=http://localhost:13001 ADMIN_PORT=13001`; stack is up — do NOT `docker compose down -v`), the metadata checks (`Complexity-actual` 1–5 = 2, Dispatch Checkpoint present, `Introduces-gate: no`, no-AC non-gating justification recorded), the atomic close-edit contract (on-approval → done + PROGRESS.md entry; on-rejection leave-review-with-reason), and the strict no-git-ops boundary.
**End:** SDET review of TASK-002-005 dispatched (one dispatch this turn — final dispatch node). On approval (→`done`, `Completed-at` set, PROGRESS.md entry): main session commits the `@demo` **spec** (`apps/admin/e2e/demo/services-catalog.demo.spec.ts`) to `brief-002-services-catalog-management` (app code, rides the slice PR; named files only, never `main`) and **holds the gallery** (`docs/demos/EPIC-002/` PNGs + `DEMO.md`) for the closing docs-lane PR per DEMO-POLICY. **Then ALL dispatch nodes are done** → I enter the post-dispatch cascade: **Audit** (Overwatch per-task rule compliance / scope-creep / efficiency over the integrated slice) → **Review** (IO design scan of the integrated `git diff` vs. the brief + cited ADRs) → **Smoke** (SDET container smoke against the Docker stack — already healthy + exercised by the e2e, but the Smoke contract still runs) → **Validate** (SDET acceptance-validation gate vs. the 7 ACs under the gherkin/e2e methodology + CI gate + quality audit) → **Close-prep** (consistency gate, archive, HANDOFF-002 + RETRO-002, move to `## Awaiting PR merge`, request PR). On rejection: route the reject reason back to the webapp-developer (Stuck-Loop counter — TASK-002-005 first review, count 1). Dispatch prompt in the `## Next Dispatch` block.

---

### SDET Review — TASK-002-005 — BRIEF-002 — 2026-06-16
**Start:** Review of TASK-002-005 (final dispatch node — `@demo` walkthrough spec + screenshot gallery for jane-accountant's services-catalog management happy path). Status `review`, `Complexity-actual: 2`. Non-gating demo artifact per DEMO-POLICY.
**Actions:**
- Read ENGINE.md, sdet.md, DEMO-POLICY.md, TASK-002-005 task file + Work Log.
- Read `apps/admin/e2e/demo/services-catalog.demo.spec.ts` (the delivered spec) and `apps/admin/package.json` (e2e script definitions).
- Read `docs/demos/EPIC-002/DEMO.md` and `docs/demos/EPIC-004/DEMO.md` (convention model).
- **Independent sha256sum recomputed** against `docs/demos/EPIC-002/*.png`: 4 distinct hashes (28e9b311 / 2b7d1542 / db953a26 / 94519cf5). No byte-identical pairs. EPIC-004 06≡08 lesson honored.
- **Visual spot-check of all 4 PNGs** (Read tool, each image): 01=catalog list (heading + table), 02=add-result (new active row visible), 03=edit-result (updated row visible), 04=deactivate-result (inactive-badge visible). Each shows a visibly distinct, correct state matching its AC-tagged name. No stale/duplicate captures.
- **Assert-before-screenshot:** every test asserts the target element visible before `page.screenshot(...)`. Titles are AC-tagged. PASS.
- **`@demo` structural exclusion:** `package.json` `e2e:run` uses `--grep-invert @demo`; `e2e:demo` uses `--grep @demo`. Non-gating is structural. PASS.
- **EPIC-004 convention mirror:** directory, naming, DEMO_DIR depth, shot() helper, fixtures, fullPage:true — all match. PASS.
- **DEMO.md:** persona `jane-accountant`, flow `flow-engagement-request` linked correctly; 4 sections with matching AC-id headers; all 4 images embedded; regenerate footer present. PASS.
- **Mandatory metadata:** `Complexity-actual: 2` (in range), `Started-at`/`Complexity-estimate` set, Dispatch Checkpoint "Starting implementation" entry present at 2026-06-16T16:37:57Z, all required spec fields present, dev Quality Gate boxes ticked (Security review N/A — capture only). PASS.
- Performed atomic close edit on TASK-002-005: ticked SDET Review box, wrote `## SDET Review` prose section (per-focus verdicts + recomputed hashes), appended Work Log approval breadcrumb, set `Completed-at: 2026-06-16T17:30:00Z`, flipped `Status: review → done`.
**End:** TASK-002-005 **approved and marked `done`**. All 5 review foci pass. Final dispatch node closed. ALL BRIEF-002 dispatch nodes are now `done`. Ready for the post-dispatch cascade: Audit (Overwatch) → Review (IO design scan) → Smoke (SDET container smoke) → Validate (SDET acceptance + CI gate) → Close-prep.
...
### SDET Review — TASK-002-004 — BRIEF-002 — 2026-06-16
**Start:** SDET review of TASK-002-004 (admin e2e catalog journeys + .feature mirror + cross-surface loop). Task status `review`, `Complexity-actual: 4`, `E2e-required: yes`. Docker pre-flight first; re-execute the specs independently against the rebuilt container stack; apply all 7 focus-area checks; close atomically.
**Actions:**
- Read ENGINE.md, sdet.md, PROGRESS.md, TASK-002-004 task file + full Work Log. Read all three spec files and the `.feature` file. Read BRIEF-002 full (incl. `acceptance_scenarios`, `methodology`, `extra_gates`). Read EPIC-002 planning file (canonical 7 gherkin scenarios + AC ownership). Read ADR-012 (testing pyramid / tier-6 applicability).
- **Docker pre-flight:** Docker 29.4.1 running. `http://localhost:13001/healthz` → 200 `{"status":"ok","app":"admin",...}`. `http://localhost:3000/healthz` → 200 `{"status":"ok","app":"portal",...}`. Stack confirmed healthy.
- **Mandatory rejection checks (all clear):** Dispatch Checkpoint entry present ("Starting implementation" is the first Work Log entry, status + `Started-at` + `Complexity-estimate` in same edit); `Started-at`/`Complexity-estimate`/`Complexity-actual` all set and in-range (4); required spec fields present (`**Acceptance criteria:**`, `**Upstream refs:**`, `**Introduces-gate: no**`); all 4 dev-owned Quality Gate boxes ticked; `SDET Review` box was open (correct); `E2e-required: yes` satisfied with genuine execution output (3× 17/17 with named tests and timing); no tool-hygiene violations in any Work Log entry.
- **SDET independent re-execution #1 (`--grep services-catalog`):** **17/17 PASS** (3.9s) — tests 12–17 all green (the 5 new catalog specs); tests 1–11 green (no regression on existing suite).
- **SDET flake re-run #2 (`--grep services-catalog`):** **17/17 PASS** (3.8s) — zero flakes.
- **Full-suite spot-check (no grep filter, `--grep-invert @demo`):** **17/17 PASS** (3.8s) — zero regressions across entire admin e2e suite. Total SDET test executions: 51 (3 × 17), all green.
- **F1 — AC↔test traceability:** PASS. Each of the 5 new spec tests carries its AC id(s) in the title and `test.describe` name. The 3 paired journeys are double-tagged (`[AC-DOOR-002-01][AC-DASH-010-01]`, `[AC-DOOR-002-02][AC-DASH-010-02]`, `[AC-DOOR-002-03][AC-DASH-010-03]`). The cross-surface test carries `[AC-DOOR-002-03]`. Both -05 tests carry `[AC-DOOR-002-05]`. All 6 in-scope ACs have ≥1 tagged covering test. AC-DOOR-002-04 correctly absent.
- **F2 — Gherkin binding, no drift:** PASS. The `.feature` file reproduces all 6 EPIC-002-canonical gherkin scenarios verbatim (Given/When/Then text matches BRIEF-002 §Acceptance scenarios byte-for-byte). The 7th `.feature` scenario (cross-surface loop, `@AC-DOOR-002-03`) is additive — it mirrors the brief's `extra_gates` cross-surface obligation, correctly tagged as EVIDENCE for AC-DOOR-002-03, explicitly disclaimed as NOT a sign-off on AC-DOOR-002-04. The brief's YAML `acceptance_scenarios` line says "bind the 7 scenarios reproduced below" but then its §Acceptance scenarios section lists only 6; the 7th is additive from the extra_gate, not contradictory drift. No mandatory scenario is missing or misrepresented.
- **F3 — Real container stack + mock provider:** PASS. e2e runs against admin container at :13001 and portal at :3000 (not dev servers). `setupAccountantSession` drives the mock auth provider — no real Clerk contacted. Write-journey timings (add 286ms, edit 461–509ms, deactivate 395–461ms) confirm real DB round-trips through the Prisma `withRequestContext` path and the `sec.fn_service_write_access` security policy.
- **F4 — Cross-surface honesty + determinism:** PASS. `services-catalog-cross-surface.spec.ts` is tagged `[AC-DOOR-002-03]` only. File header, test comments, and `.feature` scenario comment all explicitly disclaim AC-DOOR-002-04. The test creates a uniquely-named service (own `Date.now()`+random suffix), verifies portal presence, deactivates, and asserts portal absence — 4-step owned-data round-trip, deterministic, no seed dependency, no race.
- **F5 — AC-DOOR-002-05 UI-surface complement:** PASS. CLIENT test observes 307/308 redirect + confirms final URL is not admin `/services` (disjunctive assertion matches actual middleware behavior: CLIENT → redirect to portal). Anonymous test: observes redirect + asserts `onSignIn` + adds a second `expect(false)` guard for "served /services without redirect." Both are genuine redirect-observed assertions with meaningful guards. The trust-boundary proof remains TASK-002-001 tier-3; these are the correct UI-surface complement.
- **F6 — Platform-frontend scope (ADR-006):** PASS. Catalog management is admin-only. The only portal-side assertion is the cross-surface loop (a public read check, not a portal write surface). Documented single-surface exception — no portal management mirror expected per ADR-006. Not flagged as missing parity.
- **F7 — Metadata + gate discipline:** PASS. `Complexity-actual: 4` (in range). `Introduces-gate: no` correct (the admin e2e gate is a pre-existing brief-mandated gate, not newly introduced). `E2e-required: yes` evidence is genuine (3× 17/17 in Work Log with named tests, timing, and health-check output — not fabricated/curl/"written but not run"). Flake guard documentation (51 total executions, zero flakes across 3 runs) present.
- **Security:** Mock-provider pattern isolates e2e from real credentials. CLIENT/anonymous redirect tests prove middleware-level write boundary. No new dependencies introduced in spec files. No injection surface.
- **Convention compliance:** Specs follow established admin e2e structure. `uniqueName()` prevents cross-test DB interference. `page.on("dialog", accept)` correctly handles the deactivate confirm dialog. `clearSession` called in all `afterEach` hooks.
- **Performed atomic close edit:** ticked SDET Review Quality Gate box, wrote `## SDET Review` prose section, appended Work Log approval breadcrumb, set `Completed-at: 2026-06-16T16:42:00Z`, flipped `Status: review → done`. Also updated task table row (see `## Current initiative` below).
**End:** TASK-002-004 **approved and marked `done`**. All 7 review focus items pass. Independent re-execution: 3 × 17/17 (51 total), zero flakes. Full admin suite clean. AC-id test-tag contract satisfied for COVERAGE write-back on all 6 in-scope ACs. Tier-6 acceptance proof is complete. Ready for IO to: (1) update TASK-002-004 row in task table to `done`; (2) recommend main-session commit of the three spec files to `brief-002-services-catalog-management` (named files: `apps/admin/e2e/specs/services-catalog.spec.ts`, `apps/admin/e2e/specs/services-catalog-cross-surface.spec.ts`, `apps/admin/e2e/features/services-catalog.feature`); (3) dispatch TASK-002-005 (the `@demo` walkthrough gallery — final dispatch node).

---

### IO Dispatch (SDET review of TASK-002-004) — BRIEF-002 — 2026-06-16
**Start:** Re-invoked with TASK-002-004 returned at `review` — GREEN. After BUG-002-001/-002/-003 all landed and the main session rebuilt the container stack, the developer re-ran the unchanged specs: **17/17 PASS, 3 sequential runs, zero flakes** (51 total executions). The 4 write journeys (`[AC-DOOR-002-01]` add, `[AC-DOOR-002-02]` edit, `[AC-DOOR-002-03]` deactivate) + the `[AC-DOOR-002-03]` cross-surface loop now pass; the AC-DOOR-002-05 UI-surface tests still pass — specs were correct all along (the prior FAILs were the three container-runtime defects, not spec defects). Static gates clean; Complexity-actual 4. This is the slice's **tier-6 acceptance proof** — composing the SDET review.
**Phase-transition reflex:** stayed within Dispatch (no phase boundary — the slice's final acceptance-proof task review). Updated `## Current initiative` Phase line + the TASK-002-004 task-table row (`blocked` → `review (SDET dispatched)`, 17/17 GREEN note); appended this entry. (Prior session entries roll to PROGRESS-ARCHIVE.md at the next phase boundary — the Review phase transition after -005 — not mid-Dispatch.)
**Actions:**
- **IO pre-review read of TASK-002-004 + the full Work Log + spec.** Confirmed task-file consistency: status `review`, `Complexity-actual: 4` (in range), `Started-at` set, Dispatch Checkpoint "Starting implementation" entry present, required spec fields present (`**Acceptance criteria:**` the 6 in-scope ACs + the -05 UI surface, `**Upstream refs:**` ADR-006/-010/-012 + EPIC-002, `**Introduces-gate: no**` — e2e is a pre-existing brief-mandated gate, not newly introduced), all four dev-owned Quality Gate boxes ticked, `SDET Review` box open (correct). The Work Log carries ACTUAL e2e execution output (17/17) + the 3-run flake-guard evidence (no fabrication) — the `E2e-required: yes` proof obligation is met in-file.
- **Surfaced the load-bearing review focus for the tier-6 acceptance proof:** (1) AC↔test traceability — the 7 gherkin scenarios bound to AC-id-tagged specs (DOOR+DASH double-tag on the 3 paired journeys), the `.feature` mirror reproduces all 7 verbatim, no scenario drift; (2) real container stack + mock provider, NOT a dev server — SDET re-executes against admin :13001 + portal :3000 and may do its own flake re-run; (3) cross-surface loop tagged as EVIDENCE for AC-DOOR-002-03, NOT claiming EPIC-001's AC-DOOR-002-04, and deterministic (own-created uniquely-named service, not seed-dependent); (4) AC-DOOR-002-05 UI-surface tests (CLIENT/anonymous have no catalog-write UI — the complement to the TASK-002-001 tier-3 DB proof); (5) platform-frontend scope — catalog management is admin-only (ADR-006), the only portal-side assertion is the cross-surface read, no portal management mirror (documented single-surface exception, do NOT flag missing parity); (6) metadata/Dispatch-Checkpoint/`E2e-required: yes`-with-actual-output/flake-guard-evidence.
- **Composed the SDET review dispatch** for TASK-002-004 with: ENGINE.md + sdet.md reads, role tag, full task spec ref + the 7 ACs + upstream refs, the AC-id test-tag contract (HARD for COVERAGE write-back), the gherkin no-drift check + the verbatim `.feature` mirror, the re-execute-against-the-live-rebuilt-stack instruction (`ADMIN_BASE_URL=http://localhost:13001 PORTAL_BASE_URL=http://localhost:3000 pnpm --filter admin e2e:run -- --grep 'services-catalog'`) + an SDET flake re-run + the full-suite/cross-app no-regression spot-check, the cross-surface honesty check (-03 evidence not -04 sign-off; deterministic own-created service), the AC-DOOR-002-05 UI-surface complement check, the ADR-006 single-surface exception (do NOT flag missing portal mirror), the mocked-provider-not-mocked-gate check (real Clerk never contacted), the env caveat (stack up + healthy 200; admin is 13001 NOT 3001; do NOT `docker compose down -v`), the metadata/Dispatch-Checkpoint/`Introduces-gate: no`/`E2e-required: yes`-actual-output checks, the atomic close-edit contract, and the on-rejection leave-review-with-reason + BUG-file path. NO git ops by the SDET — main session commits after approval.
**End:** SDET review of TASK-002-004 dispatched (one dispatch this turn) — see the `## Next Dispatch` block. On approval (→`done`, `Completed-at` set, PROGRESS.md entry): main session commits the e2e specs to `brief-002-services-catalog-management` (named files: the two `.spec.ts` + the `.feature`, never `main`), then I compose the final dispatch — TASK-002-005 (the `@demo` walkthrough gallery → `docs/demos/EPIC-002/`; depends on -004; the last dispatch node before Review). On rejection: route the reject reason back to the webapp-developer (Stuck-Loop counter — TASK-002-004 first review, count 1). Dispatch prompt in the `## Next Dispatch` block.

<!-- Swept at Audit→Review transition (BRIEF-002, 2026-06-16) -->

### IO Audit — BRIEF-002 — 2026-06-16
**Start:** Re-invoked with **ALL BRIEF-002 dispatch nodes `done`** — TASK-002-001/-002/-003/-004/-005 + BUG-002-001/-002/-003 all SDET-approved and committed on `brief-002-services-catalog-management` (8 commits `c500053`→`f8f5405`). Dispatch exit condition met (zero tasks at `backlog`/`in-progress`; every Work Log carries submission-gate evidence; no `Escalated: yes` without recorded resolution). Entering the post-dispatch cascade at the **Audit** phase: spawn Overwatch for a read-only audit of the whole slice (integrated diff `git diff main...brief-002-services-catalog-management`, all 8 commits).
**Phase-transition reflex (Dispatch→Audit):** swept the 24 Dispatch-phase session entries (lines 124–482) to `PROGRESS-ARCHIVE.md` under a "Swept at Dispatch→Audit transition" marker; updated `## Current initiative` Phase line (Dispatch→**Audit**) + flipped the TASK-002-005 task-table row to `done`; appended this entry.
**Actions:**
- **Confirmed slice git facts for the audit dispatch (read-only):** `git log main..brief-002` = **8 commits, single committer** (`Chris Cox <chris.cox@jasgr-software.com>` — the main-session git user; **no agent committed/pushed** — boundary-compliant). Diffstat: 44 files, +6882/−92. Gated-path changes (`apps/admin`, `apps/portal` Dockerfiles, `packages/db`, `packages/auth`, `db/policies/`, `docker-compose.yml`, `prisma/schema.prisma`, `apps/*/next.config.mjs`) + the 3 BUG unblocker touches into EPIC-004 code (`packages/auth/src/{select,bindings/mock}.ts`, `packages/db/src/client.ts`) + ops doc (`runbook.md`) + task pipeline docs.
- **Composed the Overwatch audit dispatch** (read-only — reports findings, modifies nothing) covering: scope discipline, boundary compliance, platform-frontend scope, the 3 BUG escalations, container-smoke-env-block masking, task-file metadata hygiene, gate-authoring evidence, AC-id test-tag contract completeness, and rule drift.
**End:** Overwatch audit dispatched. On return: the IO dispositions every finding then proceeds to **Review** → **Smoke** → **Validate** → **Close-prep**.

### IO Audit (re-dispatch) — BRIEF-002 — 2026-06-16
**Start:** Re-invoked to drive the **Audit** phase. The prior IO turn composed the Overwatch audit dispatch but it had **not yet returned** — re-composing and handing the read-only audit to the main session. Slice git facts re-verified: 8 commits, single committer (`Chris Cox`); diffstat **44 files, +6882/−92**.
**Actions:**
- **Re-confirmed the changed-file set (44 files).** Gated-path application changes: `apps/admin/**`, `apps/portal/Dockerfile` + `next.config.mjs`, `packages/db/**`, `packages/auth/**`, `db/policies/0002-service-readable.sql`, `docker-compose.yml`, `prisma/schema.prisma`. Ungated/docs: ops docs, task pipeline files, `.env.example`.
- **Flagged two upstream-layer touches for the audit to scrutinize:** `.architecture/decisions/ADR-003-*.md` and `.implementation/OPEN-QUESTIONS.md` — PROGRESS records the ADR-003 §3 `@read_only` change as an architecture-authored amendment (BUG-002-003), not a team edit; Overwatch must confirm the team did not itself author/edit an upstream ADR.
**End:** Overwatch read-only audit re-dispatched. On return → IO dispositions each finding, records the audit verdict, transitions to **Review**.

<!-- Swept at Smoke→Validate transition (BRIEF-002, 2026-06-16) -->

### IO Audit-disposition + Review (design scan) — BRIEF-002 — 2026-06-16
**Start:** Overwatch read-only audit returned. **Verdict: 0 blocking, 1 advisory, 4 observations. Primary risk (ADR-003 upstream boundary + ADR-005 write-predicate realization) CLEAN.** No blocking finding → no fix task required before Review (Audit exit condition vacuously satisfied for the blocking set). Disposition each finding, then run the IO-owned Review design scan.
**Phase-transition reflex (Audit→Review):** swept both `### IO Audit` session entries to `PROGRESS-ARCHIVE.md` under a "Swept at Audit→Review transition" marker; updated `## Current initiative` Phase line (Audit→**Review**); recorded the Obs-2/3/4/5 retro items in `## Open retro action items`; appended this entry.
**Audit dispositions:**
- **Advisory 1 — TASK-002-001 timestamp inversion (ACTED — fixed).** `Completed-at: 2026-06-16T07:51:00Z` was chronologically BEFORE `Started-at: 2026-06-16T12:01:42Z` → negative duration. Root cause = wrong-timezone write of `Started-at` (the only `12:xx` value; all logged work — Gate-Authoring test run `07:39:10`, SDET review `07:51:00Z` — clusters in the `07:xx` UTC window). **Corrected `Started-at` → `2026-06-16T07:01:42Z`** (same-hour window, ~50min non-negative duration to `Completed-at`), with an inline `<!-- IO Audit-disposition -->` note recording the original value + rationale. In-scope IO task-file metadata edit; uncommitted, rides the slice PR. `.implementation/tasks/TASK-002-001-service-write-boundary-policy.md`.
- **Observation 2 — TASK-002-003 `Started-at: 2026-06-16T00:00:00Z` midnight sentinel.** Placeholder, in range, no gate tripped. Recorded as a metric-integrity retro item (RETRO-002 candidate). No action now.
- **Observation 3 — stale test-helper comments in `packages/db/src/service.rls.test.ts` (~L70–72, ~L88).** Comments cite `@read_only = 1` + "ADR-003 §4 pool hygiene" — both factually wrong post-BUG-002-003/Amendment-1. **Functional behavior correct; comment text only.** **DECISION: lighter path — defer to a non-blocking follow-up, NOT a micro-dispatch.** Rationale: `packages/db` is a gated path the IO/main-session may not edit directly, but driving a full task→developer→submission-gate→SDET-review pipeline to correct two comment lines (zero behavior change, zero gate impact) is disproportionate. The correction rides the next `packages/db` task that touches this file, or a dedicated doc-drift cleanup if none arises. Recorded in `## Open retro action items` + flagged for HANDOFF-002 as a known comment-drift follow-up.
- **Observations 4 + 5 — full-`ci:local` ambiguity + rule-sunset candidates (`--no-verify` clause, `PushNotification` spam-loop guard).** Recorded as RETRO-002/Close-prep retro material. No action now.
**Review — IO design scan:** read `git diff main...brief-002` against the brief's cited binding ADRs. **ADR-003 (SESSION_CONTEXT request-scoped wrapper; post-Amendment-1 `@read_only` removal) — PASS. ADR-005 (split read/write RLS predicates; CLIENT rejected from the write block) — PASS. ADR-006 (catalog management admin-only; not reachable from `apps/portal`) — PASS.** **Review verdict: PASS** — integrated diff honors ADR-003/-005/-006; zero blocking scan findings → no fix-forward task; Review exit condition met.
**End:** Audit dispositioned; Review design scan PASS. Transitioned to **Smoke**: dispatched the SDET for the container smoke gate (Docker stack only). Dispatch prompt in the `## Next Dispatch` block.

### SDET Smoke — BRIEF-002 — 2026-06-16T17:10:00Z
**Start:** Dispatched for container smoke gate (Review → Validate transition). Stack reported up + healthy with BRIEF-002 fixes applied. Docker daemon confirmed available (v29.4.1). Ran manual checks per CLAUDE.md § Container smoke test; validated against the already-running rebuilt stack (`scripts/smoke-test.sh` reviewed but not used as primary runner — it hardcodes the in-container admin port 3001 not host 13001, and would `docker compose up -d` risking the known P3019 bootstrap fragility).
**Clean-slate caveat:** `docker compose down -v` was NOT run — a clean-volume re-bootstrap would hit the known P3019 / `prisma db push` infra blocker (carried retro item; pre-dates BRIEF-002). Deployment-layer validation basis is the already-running rebuilt stack. Deviation known, documented, retro carried.
**Infrastructure: CONDITIONAL PASS** — admin/portal/azurite/mailhog all `(healthy)`; SQL reachable host 14330 + via `taxportal_user`; `sec.fn_service_write_access` exists (SQL_INLINE_TABLE_VALUED_FUNCTION); `pol_Service` = 4 BLOCK→`fn_service_write_access` + 1 FILTER→`fn_service_access` (ADR-005 split confirmed at DB); `taxportal_user` principal exists; Mailhog reachable 18025. **One caveat:** `sqlserver` compose status `(unhealthy)` — healthcheck `sqlcmd -U sa -P $MSSQL_SA_PASSWORD` fails (Error 18456 State 8) because `MSSQL_SA_PASSWORD` only applies at volume-init and the persisted volume's SA password differs; NOT a BRIEF-002 regression; DB fully operational via app principals (`taxportal_user`/`taxportal_admin` are SQL logins, not SA-gated). Same P3019/bootstrap-fragility carried infra item.
**UI: PASS** — admin `:13001/healthz` 200; portal `:3000/healthz` 200 + root `<title>Tax Accountant Client Portal</title>`; admin `/services` unauth → 307 sign-in (correct gate, not 404/500); with mock session → **HTTP 200**, `<title>Services Catalog | Tax Portal</title>`, 54KB, no `PrismaClientInitializationError`/`libssl.so.1.1` (BUG-002-002 musl-engine fix confirmed in-container — engine loads from `PRISMA_QUERY_ENGINE_LIBRARY=/app/.prisma/client/libquery_engine-linux-musl-openssl-3.0.x.so.node` override); security headers + CSP present; no CORS errors.
**Escalation to IO:** the `sqlserver` `(unhealthy)` status is a pre-existing infra issue (SA password env vs. persisted volume) — same P3019/bootstrap-fragility item, NOT introduced by BRIEF-002. Recommend adding to RETRO-002 as a new manifestation of the carried infra item; DB fully operational; does not block Validate.
**End:** Smoke gate checks complete. SDET does NOT tick the Container Smoke box — IO ticks it. Ready for Validate (SDET acceptance + CI gate + quality audit).


---

## Swept at Validate→Close-prep transition (BRIEF-002, 2026-06-16)

### IO Smoke-disposition + Validate (start) — BRIEF-002 — 2026-06-16
**Start:** SDET Container Smoke gate returned — **UI PASS, Infrastructure CONDITIONAL PASS**. IO owns the gate box. Ticking gate 5, recording the disposition, then advancing to **Validate** (gates 6 + 7 + quality audit).
**Phase-transition reflex (Smoke→Validate):** swept the prior `### IO Audit-disposition + Review` and `### SDET Smoke` session entries to `PROGRESS-ARCHIVE.md` under a "Swept at Smoke→Validate transition" marker; updated `## Current initiative` Phase line (Review→**Validate**) + added the gate scorecard line (gate 5 ✅); recorded the `sqlserver (unhealthy)` healthcheck observation in `## Open retro action items`; appended this entry.
**Container Smoke gate (5) — TICKED ✅. Disposition:**
- **UI: PASS** — admin `:13001/healthz` + portal `:3000/healthz` 200; admin `/services` renders HTTP 200 (`<title>Services Catalog | Tax Portal</title>`, 54KB) under mock auth; unauth `/services` → 307 sign-in (correct gate, not 404/500); security headers + CSP present; no CORS errors. **BUG-002-002 musl-engine fix confirmed in-container** (Prisma loads from the `PRISMA_QUERY_ENGINE_LIBRARY` override; no `PrismaClientInitializationError`/`libssl.so.1.1`).
- **Infrastructure: CONDITIONAL PASS** — admin/portal/azurite/mailhog `(healthy)`; SQL reachable 14330 + via `taxportal_user`; ADR-005 DB objects all present (`sec.fn_service_write_access` inline TVF; `pol_Service` = 4 BLOCK→write predicate + 1 FILTER→read predicate); `taxportal_user` principal exists; Mailhog 18025.
- **Clean-slate intentionally skipped:** `docker compose down -v` not run — avoids the carried P3019 / `prisma db push` bootstrap-fragility infra item; validated against the already-running rebuilt stack. Deviation documented; retro carried.
- **`sqlserver (unhealthy)` healthcheck SA mismatch:** healthcheck uses `$MSSQL_SA_PASSWORD` which only applies at volume-init; the persisted volume's SA password differs (Error 18456 State 8). **NOT a BRIEF-002 regression — DB fully operational via app principals; all EPIC-002 DB objects verified present.** Recorded as a RETRO-002 item + a new manifestation of the carried infra item (see `## Open retro action items`). **Non-blocking for Validate** (IO dispositioning authority; smoke exit condition met: infra + UI verdicts recorded, no failing check requiring a fix task).
**Validate dispatch composed (this turn):** SDET acceptance-validation gate (every in-scope AC has its AC-id-tagged covering test green at the mandated tier) + CI gate (required-CI-equivalent — `lint-and-typecheck` + `security-scan` — validated locally; **no PR open yet**, the engine opens it at Close-prep, so the GitHub required-CI green is confirmed by the Conductor post-Close-prep) + quality parity audit + `@demo` confirmation. Dispatch prompt in `## Next Dispatch`.
**End:** Container Smoke gate ✅ ticked + dispositioned. Validate dispatch handed to the main session. On return: IO records gates 6/7/quality-audit verdicts → if all pass, transition to **Close-prep** (HANDOFF-002 + RETRO-002, archive task files, slice → `## Awaiting PR merge`, engine opens the PR).

---

### SDET Validate (start) — BRIEF-002 — 2026-06-16

**Start:** Beginning three-gate Validate pass: (6) acceptance-validation gate, (7) CI gate, quality parity audit. Stack confirmed live: admin `:13001` (healthy), portal `:3000` (healthy), sqlserver `:14330` (unhealthy healthcheck — known SA mismatch, DB operational), azurite + mailhog healthy.

---

### SDET Acceptance-validation gate (gate 6) — BRIEF-002 — 2026-06-16

**Verdict: CONDITIONAL PASS (pending BUG-002-004 fix)** — all 7 in-scope ACs have tagged covering tests that are green at the mandated tier when executed with their required environment. One regression (BUG-002-004) prevents the full CI `pnpm -r test` from going clean, requiring a fix before Close-prep can proceed.

**AC → tier → artifact → result map:**

| AC | Tier | Artifact | Green? | AC-id in title? |
|---|---|---|---|---|
| AC-DOOR-002-05 | 3 (write-boundary) | `packages/db/src/service.rls.test.ts` | 10/10 ✅ | Yes — all 10 tests prefixed `[AC-DOOR-002-05]` |
| AC-DOOR-002-01 | 3 (persistence) | `packages/db/src/service.persistence.test.ts` | 4/4 ✅ | Yes — `[AC-DOOR-002-01]` in title |
| AC-DOOR-002-02 | 3 (persistence) | same file | same | Yes — `[AC-DOOR-002-02]` |
| AC-DOOR-002-03 | 3 (persistence) | same file | same | Yes — `[AC-DOOR-002-03]` |
| AC-DOOR-002-01 + AC-DASH-010-01 | 6 (e2e) | `apps/admin/e2e/specs/services-catalog.spec.ts` test 13 | 1/1 ✅ | Yes — `[AC-DOOR-002-01][AC-DASH-010-01]` dual-tagged |
| AC-DOOR-002-02 + AC-DASH-010-02 | 6 (e2e) | same, test 14 | 1/1 ✅ | Yes — `[AC-DOOR-002-02][AC-DASH-010-02]` dual-tagged |
| AC-DOOR-002-03 + AC-DASH-010-03 | 6 (e2e) | same, test 15 | 1/1 ✅ | Yes — `[AC-DOOR-002-03][AC-DASH-010-03]` dual-tagged |
| AC-DOOR-002-05 | 6 (e2e UI surface) | same, tests 16–17 | 2/2 ✅ | Yes — `[AC-DOOR-002-05]` |
| AC-DASH-010-01/-02/-03 | covered by dual-tagged e2e above | — | ✅ | Yes — dual-tag contract met |
| AC-DOOR-002-03 cross-surface loop | 6 (e2e) | `apps/admin/e2e/specs/services-catalog-cross-surface.spec.ts` test 12 | 1/1 ✅ | Yes — `[AC-DOOR-002-03]` tagged, NOT AC-DOOR-002-04 (correctly excluded) |

**Tier-3 re-execution (this session, against running stack):**
- `service.rls.test.ts` 10/10 PASS — CLIENT INSERT/UPDATE/DELETE each rejected with block predicate error (error 33504); null SESSION_CONTEXT INSERT rejected; ACCOUNTANT INSERT/UPDATE/DELETE each succeed; admin pool RLS-exempt. Counterfactual test structure verified: adding a CLIENT branch to `fn_service_write_access` would cause the 3 CLIENT-negative tests to pass (write succeeds), failing the gate.
- `service.persistence.test.ts` 4/4 PASS — createService (new active row, re-readable), updateService (name/description changed, survives re-read), deactivateService (active=false, NOT deleted — REQ-DOOR-002), listAllServices (both active+inactive returned).
- `session-context.pooled-reuse.test.ts` 2/2 PASS — cross-request reuse WITHOUT `$disconnect` (genuine pool reuse); resettability after BUG-002-003 drop of `@read_only`.
- **Total tier-3: 16/16 PASS.**

**Admin e2e re-execution (this session, 17/17 PASS):**
`ADMIN_BASE_URL=http://localhost:13001 ADMIN_PORT=13001 PORTAL_BASE_URL=http://localhost:3000 pnpm --filter admin e2e:run` →
- Tests 1–11: pre-existing auth-redirect + smoke specs (EPIC-004).
- Test 12: `[AC-DOOR-002-03]` cross-surface loop — deactivated service absent from portal public page.
- Tests 13–15: add / edit / deactivate journeys, each dual-tagged with DOOR + DASH ids.
- Tests 16–17: `[AC-DOOR-002-05]` UI surface — CLIENT redirected, anonymous redirected to sign-in.

**Gherkin binding check:**
- `apps/admin/e2e/features/services-catalog.feature` present with 7 scenarios — all 7 brief acceptance scenarios reproduced verbatim, each tagged `@AC-DOOR-002-NN` / `@AC-DASH-010-NN`. Per CLAUDE.md (Cucumber tooling not yet integrated), binding is prose-to-spec; the `.spec.ts` covers each scenario's observable behavior. AC-id test-tag contract complete for COVERAGE write-back. Cross-surface scenario correctly tagged `@AC-DOOR-002-03` (not `@AC-DOOR-002-04`).

**Skeptical lens — do tests actually fail if behavior regresses?**
- `service.rls.test.ts`: the counterfactual is documented inline (L27–29, L252, L350): adding `OR role='CLIENT'` to `fn_service_write_access` causes the three CLIENT-negative tests to pass (write succeeds), failing the assertions `expect(isBlockPredicateError).toBe(true)`. Gate is real.
- `service.persistence.test.ts`: tests assert specific field values after re-read via admin pool (role-independent). Removing the write repository or breaking SESSION_CONTEXT propagation would cause the `withClerkIdentity` call to throw (error 33504 from the RLS block predicate), failing the test at the Prisma call itself.
- E2e tests: each asserts specific DOM elements (role=row with service name, Active/inactive badge, form close). A broken server action would leave the form open or the row absent — the `.toBeVisible()` / `.not.toBeVisible()` assertions fail.

**AC-id tag contract check for COVERAGE write-back:** Every in-scope AC id appears in test titles or describe-block names at both the tier-3 and e2e levels. The dual-tagging (DOOR + DASH for journeys evidencing both) is implemented consistently. COVERAGE.md rows for AC-DOOR-002-01/-02/-03/-05 and AC-DASH-010-01/-02/-03 can be flipped on CI evidence.

**Regression found — BUG-002-004:** `pnpm -r test` exits non-zero. `apps/portal/src/app/(public)/sign-in/sign-in-rate-limit.integration.test.ts` 7/7 FAIL with `[packages/auth] MOCK_SESSION_SECRET must not be set to the dev default unless ALLOW_MOCK_AUTH=true`. Root cause: BUG-002-001 changed `mock.ts getSecret()` from a `NODE_ENV=production` guard to an `ALLOW_MOCK_AUTH=true` guard; the portal rate-limit integration test sets `AUTH_PROVIDER=mock` in `beforeEach` but does NOT set `ALLOW_MOCK_AUTH=true`. The test was authored under the old guard and was not updated when BUG-002-001 changed the contract. With `ALLOW_MOCK_AUTH=true` all 7 tests pass (verified: `ALLOW_MOCK_AUTH=true pnpm --filter portal test -- sign-in-rate-limit.integration.test.ts` → 7/7 PASS). Fix: add `process.env["ALLOW_MOCK_AUTH"] = "true"` to `beforeEach` and `delete process.env["ALLOW_MOCK_AUTH"]` to `afterEach` in that test file — same pattern as `packages/auth/src/session-expiry.test.ts`. BUG-002-004 filed. This is a blocking regression — the `test-portal` CI job (advisory `continue-on-error: true`) would show 7 failures; the slice's `pnpm -r test` is not clean.

**End:** Acceptance-validation gate CONDITIONAL PASS (AC contract complete; regression BUG-002-004 blocks full test-suite clean-run; gate 6 verdict withheld pending fix). BUG-002-004 filed.

---

### SDET CI gate (gate 7) — BRIEF-002 — 2026-06-16

**Verdict: BLOCKED — BUG-002-004 must be resolved first.**

**Note on PR state:** No PR exists yet. The engine opens the slice PR at Close-prep (next phase). The GitHub required-CI green is therefore confirmed by the Conductor post-Close-prep. What is adjudicated here is the required-CI-equivalent evidence locally on branch `brief-002-services-catalog-management`.

**Docker pre-flight:** PASS — `docker info` returns `ServerVersion: 29.4.1`; all tax-portal containers running (admin `:13001` healthy, portal `:3000` healthy, sqlserver `:14330` unhealthy healthcheck — known SA mismatch, non-blocking per Smoke disposition).

**lint-and-typecheck equivalent:**
- `pnpm lint` → PASS (zero errors; `apps/admin lint: Done`, `apps/portal lint: Done`)
- `pnpm type-check` → PASS (zero errors; `packages/ui`, `apps/admin`, `apps/portal` all Done)

**security-scan equivalent:**
- `pnpm audit --audit-level=high` → exit code 0; 1 moderate vulnerability (below the high+critical threshold). No high or critical CVEs. PASS.

**test-portal / test-admin / packages equivalent:**
- `pnpm --filter @tax-portal/auth test` → 124/124 PASS (7 files)
- `pnpm --filter @tax-portal/db test` → 41/41 PASS (8 files)
- `pnpm --filter admin test` → 41/41 PASS (3 files)
- `pnpm --filter portal test` → **FAIL: 7 failed | 16 passed** — `sign-in-rate-limit.integration.test.ts` 0/7 (`ALLOW_MOCK_AUTH` guard, BUG-002-004). Non-BRIEF-002-scope file, but on this branch it regresses from the BUG-002-001 guard change.

**Gate verdict:** BLOCKED. The `test-portal` equivalent check fails (7 tests). The required-CI-equivalent is NOT clean. Gate 7 withheld pending BUG-002-004 fix.

**End:** CI gate BLOCKED. BUG-002-004 is the only blocker.

---

### SDET Quality Parity Audit + Demo Confirmation — BRIEF-002 — 2026-06-16

**Verdict: PASS (non-gating demo confirmed)**

**Single-surface scope justification:** Catalog management is an ADR-006 admin-only (`apps/admin`) capability. The only `apps/portal` changes are `Dockerfile` + `next.config.mjs` (BUG-002-002 Prisma engine parity — no new write path, no new portal routes). No portal management mirror expected; this is the documented ADR-006 exception.

**E2e infrastructure — apps/admin:**
- `apps/admin/playwright.config.ts` present (confirms baseURL, testDir, Chromium, retries, timeout).
- `apps/admin/package.json` `e2e:run` script: `playwright test --grep-invert @demo` — present and correct.
- `apps/admin/e2e/` directory: `demo/`, `features/`, `fixtures/`, `specs/` — full structure present.
- `apps/admin/e2e/features/services-catalog.feature` — gherkin mirror present, 7 scenarios, all AC-tagged.

**E2e infrastructure — apps/portal:**
- Portal Playwright config and `e2e:run` inherited from EPIC-004. Not in BRIEF-002 scope to add new e2e (only cross-surface assertion is driven from the admin suite). No portal `e2e:run` new obligation for this brief. Confirmed present from prior EPIC.

**Tests-ran parity:**
- Admin e2e: 17/17 (this session). Portal changes (Dockerfile + next.config.mjs) carry no new tests; the musl-engine fix is structural — verified in Smoke.
- Both `apps/admin` and `apps/portal` unit suites ran. Admin 41/41 clean. Portal 7 failing (BUG-002-004 — filed separately).

**@demo confirmation (non-gating):**
- `docs/demos/EPIC-002/` present with 4 files: `01-AC-DASH-010-01-services-catalog-list.png` (273KB), `02-AC-DOOR-002-01-add-service.png` (288KB), `03-AC-DOOR-002-02-edit-service.png` (304KB), `04-AC-DOOR-002-03-deactivate-service-inactive.png` (312KB) + `DEMO.md`.
- sha256 hashes (4 distinct): `28e9b311…`, `2b7d1542…`, `db953a26…`, `94519cf5…` — all distinct, no byte-identical duplicates.
- AC-tagging: PNGs are named with their AC ids matching the gallery ordering contract in TASK-002-005.
- Demo spec at `apps/admin/e2e/demo/services-catalog.demo.spec.ts` — `@demo` tagged, correctly excluded from `e2e:run` (uses `--grep-invert @demo`), runnable only via `e2e:demo`. Non-gating confirmed.

**End:** Quality parity audit PASS. Demo confirmed (4 distinct-hash PNGs, DEMO.md present, non-gating). No parity gaps — admin has full e2e infrastructure; portal changes are structural-only with no new test obligation under ADR-006.

---

### SDET Validate (end) — BRIEF-002 — 2026-06-16T12:22:00Z

**Summary:** Three gates run. AC contract complete (all 7 ACs covered at mandated tiers, AC-id tags present, gherkin binding confirmed, dual-tagging correct, cross-surface loop correctly scoped). Quality parity audit PASS. Demo confirmed (non-gating). **CI gate BLOCKED by BUG-002-004** (`sign-in-rate-limit.integration.test.ts` 7 failures due to missing `ALLOW_MOCK_AUTH=true` in test lifecycle hooks — introduced by BUG-002-001 changing the `getSecret()` guard contract). **BUG-002-004 filed** (`.implementation/tasks/BUG-002-004-portal-rate-limit-test-missing-allow-mock-auth.md`). Fix is a 2-line test change; minimal and targeted. Gates 6 and 7 remain pending until BUG-002-004 is resolved.

**Action for IO:** Dispatch webapp-developer on BUG-002-004 (minimal fix — add `ALLOW_MOCK_AUTH=true` to `beforeEach`/`afterEach` in `sign-in-rate-limit.integration.test.ts`). After SDET approval of BUG-002-004: re-run `pnpm -r test` to confirm 0 failures, then re-run the CI gate check and tick gates 6 + 7 if clean. The acceptance-validation gate itself is clean — no AC is missing a test; the blocking issue is purely the test env-var regression.

---

### SDET BUG-002-004 Re-review + CI Gate Re-adjudication — BRIEF-002 — 2026-06-16T12:50:00Z

**Start:** Re-reviewing BUG-002-004 fix (webapp-developer applied) and re-running CI gate checks.

**BUG-002-004 Re-review — APPROVED**

Guard-unchanged confirmation: `git diff main...brief-002-services-catalog-management -- packages/auth/src/bindings/mock.ts packages/auth/src/select.ts` shows ONLY the already-committed BUG-002-001 changes (the `isMockAllowed()` predicate and `ALLOW_MOCK_AUTH` guard). No new modifications from BUG-002-004. `git status --short` on those two files returns blank — working tree clean for both guard files. The fail-closed guard was NOT weakened.

Fix scope confirmed minimal: `git diff HEAD -- apps/portal/src/app/(public)/sign-in/sign-in-rate-limit.integration.test.ts` shows exactly two additions — `process.env["ALLOW_MOCK_AUTH"] = "true"` in `beforeEach` (lines 110–114, with `// DECISION (BUG-002-004):` comment citing the BUG-002-001 guard change and session-expiry.test.ts pattern), and `delete process.env["ALLOW_MOCK_AUTH"]` in `afterEach` (line 128, with `// BUG-002-004 cleanup` comment). No other file modified.

Fix is real, not assertion-free: all 7 rate-limit tests continue to assert genuine ADR-022 throttle behavior — the env var merely unblocks the mock-session path; the assertions (throttled response `success: false`, `retryAfterMs > 0`, no session cookie on throttled attempt, independent per-IP budgets, generic error message) remain intact and would fail if the rate-limiter regressed.

Independent re-execution: `pnpm --filter portal test` → **23/23 PASS** (3 files: `sign-in-rate-limit.integration.test.ts` **7/7 PASS**, `ServiceChecklist.test.tsx` 6/6, `RequestForm.test.tsx` 10/10). Exit code 0. Stderr warnings re: `MOCK_SESSION_SECRET not set` are expected (dev fallback is intentional in test context; `ALLOW_MOCK_AUTH=true` permits it).

**BUG-002-004 verdict: APPROVED** — fix is correct, minimal, guard-intact, assertions genuine, 7/7 PASS confirmed independently.

**CI Gate Re-adjudication (gate 7) — PASS**

All checks run against branch `brief-002-services-catalog-management` (no PR open yet; GitHub required-CI green confirmed by the Conductor post-Close-prep):

- `pnpm lint` → **EXIT 0** — zero errors; `apps/admin lint: Done`, `apps/portal lint: Done`.
- `pnpm type-check` → **EXIT 0** — zero errors; `packages/ui`, `apps/admin`, `apps/portal` all Done.
- `pnpm audit --audit-level=high` → **EXIT 0** — 1 moderate vulnerability (below the high+critical threshold); no high or critical CVEs. PASS.
- `pnpm -r test` → **EXIT 0** — full workspace clean:
  - `@tax-portal/auth` 124/124 PASS (7 files)
  - `@tax-portal/db` 41/41 PASS (8 files)
  - `apps/admin` 41/41 PASS (3 files)
  - `apps/portal` 23/23 PASS (3 files) — **sign-in-rate-limit.integration.test.ts 7/7 PASS** (BUG-002-004 fixed)
  - Total: 229/229 PASS, 0 failures, 0 skipped.

**Slice status: fully acceptance-validated + CI-green.**

**Action for IO:** Tick gates 6 + 7. Slice is ready for Close-prep (HANDOFF-002 + RETRO-002, archive tasks, `## Awaiting PR merge`, open PR).

---

### SDET BUG-002-004 Re-review + CI Gate Re-adjudication (re-dispatch, API-500 recovery) — BRIEF-002 — 2026-06-16T13:27:00Z

**Start:** Prior dispatch completed all work but the API 500'd before returning its verdict. This re-dispatch independently re-executes all checks (non-mutating; results below confirm the prior session's findings exactly).

**BUG-002-004 Re-review — APPROVED (independently confirmed)**

Guard-unchanged: `git diff main...HEAD -- packages/auth/src/bindings/mock.ts packages/auth/src/select.ts` returns only BUG-002-001 committed changes. `git status --porcelain` shows the only uncommitted working-tree code change is `apps/portal/src/app/(public)/sign-in/sign-in-rate-limit.integration.test.ts` (plus IO ledger/doc files). No production code edited, no guard relaxation.

Fix verified in file: `beforeEach` line 114 sets `process.env["ALLOW_MOCK_AUTH"] = "true"` with `// DECISION (BUG-002-004):` comment citing BUG-002-001 guard change and the session-expiry.test.ts mirror pattern. `afterEach` line 128 deletes it with `// BUG-002-004 cleanup`. Exactly two lifecycle-hook lines; no assertion logic touched.

Assertions genuine: all 7 tests assert real ADR-022 throttle behavior (budget exhaustion, no session cookie on throttled attempt, per-IP budget independence, generic error message, `retryAfterMs > 0`). The `ALLOW_MOCK_AUTH=true` env var unblocks the mock-session path only; it does not weaken the throttle assertions. Tests would fail if the rate-limiter regressed.

Independent re-execution: `pnpm --filter portal test` → **23/23 PASS** (3 files: `sign-in-rate-limit.integration.test.ts` **7/7 PASS**, `ServiceChecklist.test.tsx` 6/6, `RequestForm.test.tsx` 10/10). Exit 0. Stderr `MOCK_SESSION_SECRET not set` dev-fallback warnings expected and correct.

**CI Gate Re-adjudication (gate 7) — PASS (independently confirmed)**

Branch: `brief-002-services-catalog-management`. No PR open yet — GitHub required-CI green confirmed by the Conductor post-Close-prep.

- `pnpm lint` → **EXIT 0** — `apps/portal lint: Done`, `apps/admin lint: Done`, zero errors.
- `pnpm type-check` → **EXIT 0** — `packages/ui`, `apps/admin`, `apps/portal` all Done, zero errors.
- `pnpm audit --audit-level=high` → **EXIT 0** — 1 moderate vulnerability (below high+critical threshold); no high or critical CVEs.
- `pnpm -r test` → **EXIT 0** — full workspace clean:
  - `@tax-portal/auth` 124/124 PASS (7 files)
  - `@tax-portal/db` 41/41 PASS (8 files)
  - `apps/admin` 41/41 PASS (3 files)
...
**Slice status: fully acceptance-validated + CI-green. Ready for IO to tick gates 6 + 7 and proceed to Close-prep.**
...
### Sweep marker — BRIEF-005 close → BRIEF-006 Plan-start transition — 2026-06-18
Slice-start phase-transition sweep. EPIC-005 (BRIEF-005) is **DELIVERED + finalized** (PR #48 → `f879da2`, all 9 applicable gates green). Its full delivery detail, the IO Plan Design record, the Smoke gate-record, the three SDET Validate gate-records, and the IO Close-prep/Close-finalize session entries are **retained in git history** at the BRIEF-005 build commits + the EPIC-005 task files (`tasks/done/TASK-005-*.md`) + `RETRO-005.md`/`HANDOFF-005.md`. The live `PROGRESS.md` `## Current initiative` is rewritten for BRIEF-006 at this transition; the EPIC-005 carried follow-ups (SEC-3 hardening, `inventory.md` Track-B drift, synthetic `Completed-at` inversion) move forward into the BRIEF-006 `## Open retro action items` carry. No per-entry session text is lost — git history + the archived task files preserve it.

---

### Sweep marker — BRIEF-006 Dispatch → Audit transition — 2026-06-19
Phase-transition sweep at Dispatch-exit. **All 6 gating tasks (TASK-006-001..006) are `done`** and committed to `brief-006-intake-questionnaire`; TASK-006-007 (@demo, non-gating) dispatched to close the Dispatch phase. The now-superseded **BRIEF-005 IO Plan / Smoke gate-record / three SDET Validate gate-records / Close-prep / Close-finalize session entries** that had been retained inline in the live `PROGRESS.md` are removed at this transition — they are a fully-delivered prior slice, preserved in git history at the BRIEF-005 build commits + `tasks/done/TASK-005-*.md` + `RETRO-005.md`/`HANDOFF-005.md` (per the BRIEF-005-close sweep marker above). The BRIEF-006 SDET review entries (TASK-006-001..006) + the BUG-006-001 fix chain are retained inline in the live `PROGRESS.md` as the current-slice gate records. No per-entry session text is lost.

---

### Sweep marker — BRIEF-006 close (PR limbo → Close-finalize) transition — 2026-06-19
Slice-close phase-transition sweep at Close-finalize. **BRIEF-006 / EPIC-006 is DELIVERED + finalized** (PR #50 → `main` @ `e55f8c5`, reviewed lane; gate 8 post-merge CI PASS — CI run `27796565080` `success` + CodeQL run `27796564765` `success`; gate 9 N/A `Brief-deploys: no`; all applicable gates 1–8 green; 7/7 in-scope AC delivered; zero `BUG-006-POST-*`). The full BRIEF-006 inline session history that had been retained in the live `PROGRESS.md` — the **IO Plan/Design record**, the **Dispatch chain**, the **SDET review gate-records** (TASK-006-001..007), the **BUG-006-001 reject→fix→re-approve chain**, the **Overwatch Audit + IO design-scan record**, the **Smoke gate-5 record**, the **SDET Validate (gates 6/7 + quality audit) record**, and the **IO Close-prep record** — is swept here at the close transition. Full per-entry session text is preserved in git history at the BRIEF-006 build commits (`brief-006-intake-questionnaire`, squashed into `e55f8c5`) + the archived task files (`tasks/done/TASK-006-*.md`, `tasks/done/BUG-006-001-*.md`) + `RETRO-006.md` (with the Post-Merge Addendum + final 9-gate scorecard) + `HANDOFF-006.md`. The live `PROGRESS.md` `## Current initiative` is rewritten to EPIC-006-delivered/next-ready-EPIC-007 at this transition; the four BRIEF-006 carried follow-ups (clock-source inversion 6th occurrence; `scripts/smoke-test.sh` defaults; `@demo` prior-epic PNG churn; AC-ONBD-001-01 EPIC-005 portal flake) remain in the live `## Open retro action items`. No per-entry session text is lost.
