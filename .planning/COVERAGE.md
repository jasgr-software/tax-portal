...
| — Phase 4 verified so far | 70 (EPIC-016 — PR#102 `345328e`; EPIC-017 — PR#104 `69d2726`; EPIC-018 — PR#106 `2f4b6d0`; EPIC-019 — PR#108 `b54a5936`) |
| — Phase 4 (EPIC-016..023) — *placed 2026-06-24; EPIC-016 + EPIC-017 verified 2026-06-25, EPIC-018 verified 2026-06-26, EPIC-019 verified 2026-06-27, rest `planned`* | 119 |
| &nbsp;&nbsp;— EPIC-016 (in-portal notification feed spine) | 20 (all `verified` — PR#102 `345328e`; AC-MSG-012-01/-02/-03 vs the ADR-023 mock seam → Phase-5 real-provider re-validation) |
| &nbsp;&nbsp;— EPIC-017 (per-engagement & general messaging) | 24 (all `verified` — PR#104 `69d2726`) |
| &nbsp;&nbsp;— EPIC-018 (email digest fallback) | 12 (all `verified` — PR#106 `2f4b6d0`; AC-MSG-008/-009/-010/-011 vs the ADR-023/-025 mock email transport (Mailhog) → Phase-5 real-ESP re-validation) |
| &nbsp;&nbsp;— EPIC-019 (overdue detection & reminder engine) | 14 (all `verified` — PR#108 `b54a5936`; auto-detection behind the ADR-023 time-injectable seam; AC-MSG-014-02 client nudge surfaces via the EPIC-016 feed / EPIC-018 digest) |
...
| AC `verified` (signed off) | **260** — all 48 remaining Phase-1 placed AC (EPIC-001 13 · EPIC-004 8 · EPIC-002 7 · EPIC-003 20; **Phase 1 / MVP complete**) **+ all 44 Phase-2 onboarding-gate AC** (EPIC-005 10 · EPIC-006 7 · EPIC-007 19 · **EPIC-008 8** — the capstone) **+ all 5 EPIC-009 sign-in-lane AC** (AC-AUTH-010-01/-02/-03 — the redirect mechanism delivered by EPIC-004 PR#38, ownership consolidated into the sign-in epic 2026-06-21, tests still pass; **+ AC-AUTH-013-01/-02 — the sign-in/sign-out capability, verified vs the MOCK provider via PR#71 `169b09e` 2026-06-21**) **+ all 25 EPIC-010 lifecycle-core AC** (LIFE-001/002/003/004/005/006, AUTH-002/003/008 — the four-stage pipeline, client-facing labels, two-confirmation completion gate, accountant reopen, full visibility + client own-data isolation + indefinite post-completion access; PR#87 `7afd312` 2026-06-22; AUTH-003-* are the **feature** AC over the Phase-2-built isolation mechanism — not double-counted) **+ all 9 EPIC-011 engagement-attributes AC** (LIFE-007/008/009 — per-engagement due date set/update, accountant-only internal notes behind the HARD `pol_EngagementNote` RLS, priority flag set/clear, each per-engagement; PR#89 `9445e36` 2026-06-23) **+ all 20 EPIC-012 creation-paths & multi-participant AC** (DOOR-009/010 returning-client + accountant-initiated creation, LIFE-010 concurrent engagements, LIFE-011 duplicate warn+override, LIFE-012/AUTH-007 multi-participant separate-accounts behind the HARD `pol_EngagementParticipant`/extended `pol_Engagement` RLS — participant isolation proven both ways; PR#93 `5883fed` 2026-06-23) **+ all 13 EPIC-013 secure-file-exchange AC** (FILE-001-01/-03/-04 accountant upload + both-party download, FILE-009-01/-02/-03 replace → newest current → prior versions retained AND accessible, FILE-010-01/-02/-03/-04 organize/create-rename-arrange/place-in-folder/accountant-only-behind-HARD-`pol`-isolated folders, FILE-011-01/-02/-03 top-level group by engagement + tax year + navigate engagement→tax-year→folder; the reviewed-lane `/pr-review` panel caught a **version-download IDOR** (blocker) the in-slice gates missed — fixed in-PR so `requestDownloadUrlForVersionAction` threads `versionId`, resolves the version under the request-pool/RLS, asserts `documentId` match, and signs only the server-resolved key, with cross-resource key-substitution negative tests on both surfaces; PR#95 `4aa26d0` 2026-06-23) **+ all 10 EPIC-014 file-deletion / soft-delete / 7-year-retention AC** (FILE-004-01/-02/-03 accountant-only delete with the no-client-delete boundary proven **both ways** — HARD `pol`-isolation RLS + a portal no-delete-path negative e2e; FILE-006-01/-02/-03 soft-delete leaves the working view / file retained-not-destroyed in-window / recoverable until retention elapses; FILE-005-01/-02/-03 retention clock 7 years from completion / in-window nothing removes incl. an accountant deletion / retention governs in-window; AC-NFR-006-01 system-enforced retention — the NFR twin of FILE-005; PR#97 `37707ad` 2026-06-24) **+ all 16 EPIC-015 post-retention-purge & legal-hold AC** (FILE-013-01..06 purge eligible only post-window / accountant-admin-only proven both ways / explicit-confirmation / never-automatic-on-expiry / eligible-but-unpurged stays retained / purge audited + record survives; FILE-014-01..07 hold on engagement / hold on client → all engagements / held = no purge even post-expiry / indefinite no-auto-expire / lift restores eligibility / place + lift each audited; FILE-015-01/-02 in-window erasure = access-revocation-only / physical destruction only post-window + no-hold + confirmed; AC-NFR-010-07 audit survives purge; the reviewed-lane `/pr-review` panel caught a **purge-atomicity blocker** the in-slice gates missed — purge statements ran off-transaction, so a mid-purge failure could destroy data while leaving no audit record; fixed in-PR so the whole purge runs inside the `withAuditTransaction` envelope and rolls back all-or-nothing, with audit-survives-purge now proven by a dedicated rollback regression test; PR#99 `53b3444` 2026-06-24). **Phase 1 + Phase 2 complete; EPIC-009 delivered (PoC mock realization); EPIC-010/011/012/013/014/015 delivered — EPIC-015 CLOSES Phase 3 (it was the last un-delivered Phase-3 epic). NOTE:** AC-AUTH-013-01/-02 (and the AUTH-010 redirect trio) are verified **against the mock auth provider** for the proof of concept — their real-provider (Clerk) re-validation is still outstanding at Phase 5 (see § Provider re-validation). **+ all 20 EPIC-016 in-portal-notification-feed-spine AC** (MSG-007-01/-02/-03 authoritative feed, MSG-012-01/-02/-03 real-time delivery, MSG-015-01/-02/-03 mark-read-on-view incl. cross-app per ADR-010, MSG-016-01/-02 ≥90-day retention behind the HARD tier-3 RLS, MSG-017-01/-02/-03 unread badge, MSG-013-03 accountant-on-upload, MSG-014-03/-04/-05/-06 client subset, MSG-014-07 the **HARD tier-3 per-viewer RLS** generalizing EPIC-003's `pol_Notification` to a CLIENT branch — bidirectional CLIENT-A↔B isolation + null-context + cross-type + the F1 ACCOUNTANT-cross-recipient read/write isolation fix, all proven; PR#102 `345328e` 2026-06-25 — **EPIC-016 is the first Phase-4 epic delivered. NOTE:** AC-MSG-012-01/-02/-03 (real-time arrival) are verified **against the ADR-023 SSE-backed mock seam** — the real-time *transport choice has no dedicated ADR yet (planning-flagged architecture gap)* and must be **re-validated against the real provider in Phase 5** (§ Provider re-validation); AC-MSG-012-01/-02 "surfaces in the feed" is verified via the **badge-increment proxy** (the feed list is SSR; the badge is the live element)). **+ all 24 EPIC-017 per-engagement-&-general-messaging AC** (MSG-001-01..04 per-engagement thread — one-per-engagement + recorded/visible-to-participants behind the HARD `pol_Thread`/`pol_Message` RLS + ordered history + both-parties read/contribute; MSG-002-01..03 accountant-initiated general thread associated-with/visible-to-the-client-only behind the HARD RLS + ordered history; MSG-003-01..03 plain-text-only — body verbatim, markup not rendered, no inline images, no `dangerouslySetInnerHTML`; MSG-004-01..05 attachments — attach one+ / visible to participants / participant-scoped short-lived signed-URL retrieval with the IDOR cross-resource key-substitution negative / available while retained / **HARD scan-before-available + upload type/size rules** reusing the EPIC-007 scan seam, infected/indeterminate never signable; MSG-005-01..04 per-viewer unread — indicator on engagement+general threads / **per-viewer HARD `pol_ThreadReadState`** state + IDOR negative / clears once seen; MSG-006-01..03 indefinite retention + **archive-on-close** wired additively into the EPIC-010 Complete transition (archive is a status flip, never a delete; archived rows stay readable) — distinct from EPIC-016's ≥90-day floor; MSG-013-02 accountant + MSG-014-01 client recipient-only new-message notification emitted onto the EPIC-016 feed spine, rendered+linked in BOTH feeds, sender never self-notified; the reviewed-lane `/pr-review` panel caught a **cross-tenant-write security blocker** the in-slice gates missed — CLIENT send/attach actions wrote on the admin pool with role-only auth; fixed in-PR so both send + attach actions gate on participation via the **request pool** (`getThreadForEngagement`/`getThreadById`/`verifyMessageInThread` under RLS) before the admin-pool write, with a red→green write-side negative test; PR#104 `69d2726` 2026-06-25 — **EPIC-017 is the second Phase-4 epic delivered. Non-blocking advisory:** the general-thread new-message *link* is covered by unit tests on both surfaces while the engagement-thread link is covered in e2e — both AC-MSG-013-02/-014-01 are satisfied; this is a coverage-depth follow-up, not a gap). **+ all 12 EPIC-018 email-digest-fallback AC** (MSG-008-01/-02/-03 the **content-free nudge** — conveys only "new activity" + a sign-in affordance, carries no message/document/client/engagement/event detail, acting on it lands at the portal sign-in; the no-PII body is the HARD tier-3 gate proven **two-sided** at composer-unit + dispatch-integration + e2e layers; MSG-009-01/-02/-03 the **daily cap** — at most one email per recipient per day on a per-recipient `lastNudgeSentAt` calendar-day-UTC watermark, never one per event, N=3 events → one nudge, HARD tier-3; MSG-010-01/-02/-03/-04 **accountant self-suppression** — she turns off her own email, suppressed → zero emails of any kind, her in-portal feed stays intact, her setting never changes whether clients are emailed; MSG-011-01/-02 **client default-on** — `emailNudgeEnabled BIT NOT NULL DEFAULT 1` ⇒ a newly created client account is nudge-enabled with no opt-in step; built **additively on** the EPIC-016 `Notification` feed + the ADR-025 `packages/email` seam (both consumed, neither rebuilt) with two additive `User` columns (no new table/RLS policy); the `/pr-fix` pass removed the `_userIdFilter` test seam, promoted `_emailProvider` to plain DI, removed the unused composer `role` param, and fixed a **watermark double-send bug** with a new regression test (`email-digest.hardening.test.ts`) — the AC-tagged tests all still exist and pass post-fix on the merge commit; PR#106 `2f4b6d0` 2026-06-26 — **EPIC-018 is the third Phase-4 epic delivered. NOTE:** the email channel is realized against the ADR-023/-025 **mock transport (Mailhog)** for the POC — real-ESP (e.g. Resend) re-validation is Phase 5 (§ Provider re-validation)). **+ all 14 EPIC-019 overdue-detection-&-reminder-engine AC** (FILE-012-01/-03/-04 the system identifies overdue document requests **on its own** — no accountant trigger — by the request's due date, behind the ADR-023 **time-injectable seam** so "becomes overdue at the due date" is deterministically asserted; FILE-012-02 the overdue request is flagged/surfaced (tier-6 admin); MSG-018-01/-02 auto-identify → raise a reminder; MSG-018-03/-04 + DASH-008-01/-02/-03 the **configurable cadence** — a global default frequency plus a **per-engagement override that takes precedence** (the precedence is the HARD tier-3 gate `reminder-cadence.precedence.test.ts`; the config journeys are tier-6 admin); MSG-013-05/-06 the accountant is notified when a request becomes **overdue** and when an engagement is **approaching its due date**; MSG-014-02 the client is notified when a **document request is created** for them — all reminder notifications emitted onto the EPIC-016 feed spine and honoring the EPIC-018 email digest, built **additively** over the EPIC-011 engagement due date + EPIC-013 document requests (consumed, not rebuilt); the 3-lens panel posted an advisory request-changes (1 major: an encrypt-default TLS downgrade) which `/pr-fix` resolved in `b2b55ce` before merge — the AC-tagged tests all still exist and pass post-fix on the merge commit; PR#108 `b54a5936` 2026-06-27 — **EPIC-019 is the fourth Phase-4 epic delivered. NOTE:** any scheduler/timer driving periodic detection is realized against the ADR-023 **mock, time-injectable seam** for the POC — a real scheduler/transport is Phase 5 (§ Provider re-validation). EPIC-019 does **not** close Phase 4 — EPIC-020..023 remain `planned` (EPIC-023 is the closer)). |
| AC still `planned` (placed, not yet verified) | **49** — Phase-4 AC for EPIC-020..023 (EPIC-016's 20 flipped to `verified` 2026-06-25 via PR#102 `345328e`; EPIC-017's 24 via PR#104 `69d2726`; EPIC-018's 12 via PR#106 `2f4b6d0` 2026-06-26; EPIC-019's 14 via PR#108 `b54a5936` 2026-06-27), placed 2026-06-24, awaiting build + CI sign-off. *(Every Phase-1 + Phase-2 + Phase-3 AC is `verified`; Phase 3 is closed — EPIC-015's 16 AC flipped 2026-06-24 via PR#99 `53b3444`; EPIC-014's 10 via PR#97 `37707ad`; EPIC-013's 13 via PR#95 `4aa26d0`; EPIC-012's 20 via PR#93; EPIC-011's 9 via PR#89; EPIC-010's 25 via PR#87; EPIC-009's 2 via PR#71.)* |
...
| REQ-MSG-007 | AC-MSG-007-01 | EPIC-016 | 4 | `AC-MSG-007-01` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `notification.integration.test.ts` + tier-3 `source-event-wiring.integration.test.ts` + tier-6 e2e; entitled notification appears in the feed |
| REQ-MSG-007 | AC-MSG-007-02 | EPIC-016 | 4 | `AC-MSG-007-02` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `notification.integration.test.ts`; feed is the authoritative/complete record |
| REQ-MSG-007 | AC-MSG-007-03 | EPIC-016 | 4 | `AC-MSG-007-03` | verified | PR#102 `345328e` (2026-06-25) — tier-6 `apps/portal/.../notification-feed.spec.ts`; other channels supplementary (verified vs feed presence — email channel is EPIC-018) |
| REQ-MSG-012 | AC-MSG-012-01 | EPIC-016 | 4 | `AC-MSG-012-01` | verified (mock seam) | PR#102 `345328e` (2026-06-25) — tier-6 e2e both surfaces via the **ADR-023 SSE-backed browser-reachable mock** (push-without-navigation badge increment); "surfaces in the feed" verified via the **badge-increment proxy** (feed list is SSR; badge is the live element). **Real-time transport has no dedicated ADR yet (planning-flagged arch gap) → re-validate vs real provider in Phase 5.** |
| REQ-MSG-012 | AC-MSG-012-02 | EPIC-016 | 4 | `AC-MSG-012-02` | verified (mock seam) | PR#102 `345328e` (2026-06-25) — tier-6 e2e both surfaces via the **ADR-023 SSE mock**; new notifications appear in real time, verified via the **badge-increment proxy** (SSR feed list). **Real-time transport choice unADR'd → Phase 5 real-provider re-validation.** |
| REQ-MSG-012 | AC-MSG-012-03 | EPIC-016 | 4 | `AC-MSG-012-03` | verified (mock seam) | PR#102 `345328e` (2026-06-25) — tier-6 e2e both surfaces; badge increments on real-time arrival behind the ADR-023 SSE mock. **Real provider re-validation → Phase 5.** |
| REQ-MSG-015 | AC-MSG-015-01 | EPIC-016 | 4 | `AC-MSG-015-01` | verified | PR#102 `345328e` (2026-06-25) — tier-6 `apps/portal/.../notification-feed.spec.ts`; notification references its triggering item |
| REQ-MSG-015 | AC-MSG-015-02 | EPIC-016 | 4 | `AC-MSG-015-02` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `notification.integration.test.ts` + tier-6 `notification-mark-read-cross-app.spec.ts` (cross-app, ADR-010); viewing linked item auto-marks read |
| REQ-MSG-015 | AC-MSG-015-03 | EPIC-016 | 4 | `AC-MSG-015-03` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `notification.integration.test.ts` + tier-6 cross-app e2e; read reflects in feed + count, no dismiss step |
| REQ-MSG-016 | AC-MSG-016-01 | EPIC-016 | 4 | `AC-MSG-016-01` | verified | PR#102 `345328e` (2026-06-25) — **tier-3** `notification.rls.test.ts` (read notification ≥90 days old retained + visible) + `notification.integration.test.ts`; history retained/viewable ≥90 days |
| REQ-MSG-016 | AC-MSG-016-02 | EPIC-016 | 4 | `AC-MSG-016-02` | verified | PR#102 `345328e` (2026-06-25) — **tier-3** `notification.rls.test.ts` (unread ≥90 days old retained + visible) + `notification.integration.test.ts`; read + unread both retained in window |
| REQ-MSG-017 | AC-MSG-017-01 | EPIC-016 | 4 | `AC-MSG-017-01` | verified | PR#102 `345328e` (2026-06-25) — tier-6 e2e both surfaces; badge present in nav from any area |
| REQ-MSG-017 | AC-MSG-017-02 | EPIC-016 | 4 | `AC-MSG-017-02` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `notification.integration.test.ts` + tier-6 e2e both surfaces; badge shows unread count |
| REQ-MSG-017 | AC-MSG-017-03 | EPIC-016 | 4 | `AC-MSG-017-03` | verified | PR#102 `345328e` (2026-06-25) — tier-6 e2e both surfaces; badge updates on read / arrival |
| REQ-MSG-013 | AC-MSG-013-03 | EPIC-016 | 4 | `AC-MSG-013-03` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `source-event-wiring.integration.test.ts` + tier-6 `apps/admin/.../notification-feed.spec.ts`; accountant notified on document upload (source EPIC-013) |
| REQ-MSG-014 | AC-MSG-014-03 | EPIC-016 | 4 | `AC-MSG-014-03` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `source-event-wiring.integration.test.ts` + tier-6 `apps/portal/.../notification-feed.spec.ts`; client notified on engagement status change (source EPIC-010) |
| REQ-MSG-014 | AC-MSG-014-04 | EPIC-016 | 4 | `AC-MSG-014-04` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `source-event-wiring.integration.test.ts` + tier-6 e2e; client notified when a deliverable is ready |
| REQ-MSG-014 | AC-MSG-014-05 | EPIC-016 | 4 | `AC-MSG-014-05` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `source-event-wiring.integration.test.ts` + tier-6 e2e; client (account-holding) notified request accepted; account-less prospect by email (EPIC-003) |
| REQ-MSG-014 | AC-MSG-014-06 | EPIC-016 | 4 | `AC-MSG-014-06` | verified | PR#102 `345328e` (2026-06-25) — tier-3 `source-event-wiring.integration.test.ts` + tier-6 e2e; client (account-holding) notified request declined; account-less prospect by email (EPIC-003) |
| REQ-MSG-014 | AC-MSG-014-07 | EPIC-016 | 4 | `AC-MSG-014-07` | verified | PR#102 `345328e` (2026-06-25) — **HARD tier-3** `notification.rls.test.ts`: bidirectional CLIENT-A↔B zero-rows + null-context zero + cross-type zero + the F1 ACCOUNTANT-cross-recipient isolation negatives (read-fix + write-fix on a shared linkedItemId); client sees only own events |
| REQ-MSG-001 | AC-MSG-001-01 | EPIC-017 | 4 | `AC-MSG-001-01` | verified | PR#104 `69d2726` 2026-06-25; exactly one thread per engagement (`@@unique[engagementId]`); tier-6 |
| REQ-MSG-001 | AC-MSG-001-02 | EPIC-017 | 4 | `AC-MSG-001-02` | verified | PR#104 `69d2726` 2026-06-25; message recorded + visible to participants; HARD tier-3 `pol_Thread`/`pol_Message` RLS both-ways |
| REQ-MSG-001 | AC-MSG-001-03 | EPIC-017 | 4 | `AC-MSG-001-03` | verified | PR#104 `69d2726` 2026-06-25; full ordered history persists across sessions; tier-6 |
| REQ-MSG-001 | AC-MSG-001-04 | EPIC-017 | 4 | `AC-MSG-001-04` | verified | PR#104 `69d2726` 2026-06-25; both accountant + client(s) read/contribute; HARD tier-3 + tier-6 |
| REQ-MSG-002 | AC-MSG-002-01 | EPIC-017 | 4 | `AC-MSG-002-01` | verified | PR#104 `69d2726` 2026-06-25; accountant starts a general (non-engagement) thread; tier-6 |
| REQ-MSG-002 | AC-MSG-002-02 | EPIC-017 | 4 | `AC-MSG-002-02` | verified | PR#104 `69d2726` 2026-06-25; general thread associated with + visible to accountant + that client only; HARD tier-3 RLS |
| REQ-MSG-002 | AC-MSG-002-03 | EPIC-017 | 4 | `AC-MSG-002-03` | verified | PR#104 `69d2726` 2026-06-25; general-thread messages retained as ordered history; tier-6 |
| REQ-MSG-003 | AC-MSG-003-01 | EPIC-017 | 4 | `AC-MSG-003-01` | verified | PR#104 `69d2726` 2026-06-25; body treated/displayed as plain text; tier-3 |
| REQ-MSG-003 | AC-MSG-003-02 | EPIC-017 | 4 | `AC-MSG-003-02` | verified | PR#104 `69d2726` 2026-06-25; markup shown verbatim — no `dangerouslySetInnerHTML` (shared `MessageBody`); tier-3 |
| REQ-MSG-003 | AC-MSG-003-03 | EPIC-017 | 4 | `AC-MSG-003-03` | verified | PR#104 `69d2726` 2026-06-25; no inline images in body; tier-3 |
| REQ-MSG-004 | AC-MSG-004-01 | EPIC-017 | 4 | `AC-MSG-004-01` | verified | PR#104 `69d2726` 2026-06-25; sender attaches one or more files; tier-6 |
| REQ-MSG-004 | AC-MSG-004-02 | EPIC-017 | 4 | `AC-MSG-004-02` | verified | PR#104 `69d2726` 2026-06-25; attachments visible to participants; tier-6 |
| REQ-MSG-004 | AC-MSG-004-03 | EPIC-017 | 4 | `AC-MSG-004-03` | verified | PR#104 `69d2726` 2026-06-25; participant retrieves via short-lived participant-scoped signed URL (IDOR cross-resource negative); tier-6 |
| REQ-MSG-004 | AC-MSG-004-04 | EPIC-017 | 4 | `AC-MSG-004-04` | verified | PR#104 `69d2726` 2026-06-25; attachment available while message retained; tier-6 |
| REQ-MSG-004 | AC-MSG-004-05 | EPIC-017 | 4 | `AC-MSG-004-05` | verified | PR#104 `69d2726` 2026-06-25; **HARD** tier-3 — scanned (NFR-009) + upload type/size rules before available; infected/indeterminate never signable (EPIC-007 scan seam) |
| REQ-MSG-005 | AC-MSG-005-01 | EPIC-017 | 4 | `AC-MSG-005-01` | verified | PR#104 `69d2726` 2026-06-25; unread indicator on thread with new messages; tier-6 |
| REQ-MSG-005 | AC-MSG-005-02 | EPIC-017 | 4 | `AC-MSG-005-02` | verified | PR#104 `69d2726` 2026-06-25; indicator on engagement + general threads; tier-6 |
| REQ-MSG-005 | AC-MSG-005-03 | EPIC-017 | 4 | `AC-MSG-005-03` | verified | PR#104 `69d2726` 2026-06-25; unread state is per-viewer (clears for one viewer only); HARD tier-3 + IDOR negative |
| REQ-MSG-005 | AC-MSG-005-04 | EPIC-017 | 4 | `AC-MSG-005-04` | verified | PR#104 `69d2726` 2026-06-25; indicator clears once seen; tier-6 |
| REQ-MSG-006 | AC-MSG-006-01 | EPIC-017 | 4 | `AC-MSG-006-01` | verified | PR#104 `69d2726` 2026-06-25; threads retained indefinitely (close ≠ delete); HARD tier-3 (archive integration 5/5) |
| REQ-MSG-006 | AC-MSG-006-02 | EPIC-017 | 4 | `AC-MSG-006-02` | verified | PR#104 `69d2726` 2026-06-25; archived (status flip) not deleted on close — wired into the EPIC-010 Complete transition; HARD tier-3 |
| REQ-MSG-006 | AC-MSG-006-03 | EPIC-017 | 4 | `AC-MSG-006-03` | verified | PR#104 `69d2726` 2026-06-25; archived thread stays fully readable; tier-6 |
| REQ-MSG-013 | AC-MSG-013-02 | EPIC-017 | 4 | `AC-MSG-013-02` | verified | PR#104 `69d2726` 2026-06-25; accountant notified of new message (recipient-only, onto EPIC-016 feed); tier-6 — *advisory: general-thread link covered by unit tests both surfaces, engagement-thread link by e2e (coverage-depth follow-up, not a gap)* |
| REQ-MSG-014 | AC-MSG-014-01 | EPIC-017 | 4 | `AC-MSG-014-01` | verified | PR#104 `69d2726` 2026-06-25; client notified of new message (recipient-only, onto EPIC-016 feed); tier-6 — *advisory: same general-thread vs engagement-thread link coverage-depth follow-up* |
| REQ-MSG-008 | AC-MSG-008-01 | EPIC-018 | 4 | `AC-MSG-008-01` | verified | PR#106 `2f4b6d0` (2026-06-26) — tier-6 `apps/portal/e2e/specs/email-nudge-signin.spec.ts` (content-free nudge in Mailhog + sign-in affordance); composer unit `packages/email/src/digest.test.ts` + tier-3 dispatch corroborate |
| REQ-MSG-008 | AC-MSG-008-02 | EPIC-018 | 4 | `AC-MSG-008-02` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE1, two-sided: 5 seeded sensitive strings absent + nudge present) + composer unit `digest.test.ts` + tier-6; content-free at all layers |
| REQ-MSG-008 | AC-MSG-008-03 | EPIC-018 | 4 | `AC-MSG-008-03` | verified | PR#106 `2f4b6d0` (2026-06-26) — tier-6 `apps/portal/e2e/specs/email-nudge-signin.spec.ts` (link lands at portal `/sign-in`) |
| REQ-MSG-009 | AC-MSG-009-01 | EPIC-018 | 4 | `AC-MSG-009-01` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE2: N=3 events → sentCount==1; next day → ==2; cap on `lastNudgeSentAt`) + `email-digest.integration.test.ts` + watermark-double-send regression in `email-digest.hardening.test.ts` |
| REQ-MSG-009 | AC-MSG-009-02 | EPIC-018 | 4 | `AC-MSG-009-02` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE2: no per-event emails) |
| REQ-MSG-009 | AC-MSG-009-03 | EPIC-018 | 4 | `AC-MSG-009-03` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE2: multiple events → one daily nudge) |
| REQ-MSG-010 | AC-MSG-010-01 | EPIC-018 | 4 | `AC-MSG-010-01` | verified | PR#106 `2f4b6d0` (2026-06-26) — tier-6 `apps/admin/e2e/specs/accountant-email-suppression.spec.ts` (toggle persists) + tier-2 `settings/notifications/actions.test.ts` |
| REQ-MSG-010 | AC-MSG-010-02 | EPIC-018 | 4 | `AC-MSG-010-02` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE3: suppressed → 0 emails of any kind) |
| REQ-MSG-010 | AC-MSG-010-03 | EPIC-018 | 4 | `AC-MSG-010-03` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE3: in-portal feed intact while email suppressed) + `email-digest.integration.test.ts` |
| REQ-MSG-010 | AC-MSG-010-04 | EPIC-018 | 4 | `AC-MSG-010-04` | verified | PR#106 `2f4b6d0` (2026-06-26) — **hard** tier-3 `packages/db/.../email-digest.dispatch.integration.test.ts` (GATE3: her suppression leaves client nudges unchanged) |
| REQ-MSG-011 | AC-MSG-011-01 | EPIC-018 | 4 | `AC-MSG-011-01` | verified | PR#106 `2f4b6d0` (2026-06-26) — tier-3 `packages/db/.../user-email-preference.integration.test.ts` + `email-digest.integration.test.ts` (default-on at the DB: `emailNudgeEnabled BIT NOT NULL DEFAULT 1`) |
| REQ-MSG-011 | AC-MSG-011-02 | EPIC-018 | 4 | `AC-MSG-011-02` | verified | PR#106 `2f4b6d0` (2026-06-26) — tier-6 `apps/portal/e2e/specs/client-emailed-without-optin.spec.ts` (emailed, no opt-in step) |
| REQ-FILE-012 | AC-FILE-012-01 | EPIC-019 | 4 | `AC-FILE-012-01` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.detection.test.ts` — overdue requests identified |
| REQ-FILE-012 | AC-FILE-012-02 | EPIC-019 | 4 | `AC-FILE-012-02` | verified | PR#108 `b54a5936` 2026-06-27; tier-6 admin `overdue-reminders.spec.ts` (+ tier-3 `document-request.overdue-flag.test.ts`) — overdue request flagged/surfaced |
| REQ-FILE-012 | AC-FILE-012-03 | EPIC-019 | 4 | `AC-FILE-012-03` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.detection.test.ts` — detection without accountant trigger (time-injectable seam) |
| REQ-FILE-012 | AC-FILE-012-04 | EPIC-019 | 4 | `AC-FILE-012-04` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.detection.test.ts` + `document-request.overdue-flag.test.ts` — overdue determined by due date |
| REQ-MSG-018 | AC-MSG-018-01 | EPIC-019 | 4 | `AC-MSG-018-01` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.detection.test.ts` — auto-identify overdue (no manual check) |
| REQ-MSG-018 | AC-MSG-018-02 | EPIC-019 | 4 | `AC-MSG-018-02` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.cadence.test.ts` — overdue request raises a reminder |
| REQ-MSG-018 | AC-MSG-018-03 | EPIC-019 | 4 | `AC-MSG-018-03` | verified | PR#108 `b54a5936` 2026-06-27; tier-6 admin `overdue-reminders.spec.ts` — global default reminder frequency |
| REQ-MSG-018 | AC-MSG-018-04 | EPIC-019 | 4 | `AC-MSG-018-04` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-cadence.precedence.test.ts` — per-engagement override takes precedence |
| REQ-DASH-008 | AC-DASH-008-01 | EPIC-019 | 4 | `AC-DASH-008-01` | verified | PR#108 `b54a5936` 2026-06-27; tier-6 admin `overdue-reminders.spec.ts` — set global default cadence |
| REQ-DASH-008 | AC-DASH-008-02 | EPIC-019 | 4 | `AC-DASH-008-02` | verified | PR#108 `b54a5936` 2026-06-27; tier-6 admin `overdue-reminders.spec.ts` — set per-engagement cadence |
| REQ-DASH-008 | AC-DASH-008-03 | EPIC-019 | 4 | `AC-DASH-008-03` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-cadence.precedence.test.ts` — per-engagement cadence wins (precedence) |
| REQ-MSG-013 | AC-MSG-013-05 | EPIC-019 | 4 | `AC-MSG-013-05` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.cadence.test.ts` + `reminder-engine.detection.test.ts` — accountant notified request overdue |
| REQ-MSG-013 | AC-MSG-013-06 | EPIC-019 | 4 | `AC-MSG-013-06` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `reminder-engine.approaching.test.ts` — accountant notified due date approaching |
| REQ-MSG-014 | AC-MSG-014-02 | EPIC-019 | 4 | `AC-MSG-014-02` | verified | PR#108 `b54a5936` 2026-06-27; tier-3 `document-request.request-created.rls.test.ts` (+ tier-6 portal `request-created-nudge.spec.ts`) — client notified a document request was created |
...
- **Real-time notification transport → Phase 5 (Production Readiness, placeholder).** Re-validates
  **AC-MSG-012-01/-02/-03** (owned by EPIC-016 — real-time arrival + badge increment) against the **real
  real-time provider** when chosen. These AC are `verified` for the POC against the **ADR-023 SSE-backed
  browser-reachable mock seam** (push-without-navigation badge increment; "surfaces in the feed" verified
  via the **badge-increment proxy** because the feed list is SSR and the badge is the live element).
  **Planning-flagged architecture gap:** the real-time *transport choice has no dedicated ADR yet* — this is
  an honest mock-seam-phase verification, not a silent full sign-off. The Phase-5 enablement slice owns no
  new coverage rows; EPIC-016's rows stay `verified` and the real-provider confirmation is recorded here.
  **Not minted as an epic yet.**
- **Real email transport (ESP) → Phase 5 (Production Readiness, placeholder).** Re-validates **EPIC-018's
  email-digest-fallback AC** (AC-MSG-008-01/-02/-03, AC-MSG-009-01/-02/-03, AC-MSG-010-01/-02/-03/-04,
  AC-MSG-011-01/-02 — content-free nudge, daily cap, accountant self-suppression, client default-on) against
  a **real email provider** when chosen. These AC are `verified` for the POC against the **ADR-023/-025
  mock SMTP transport (Mailhog)**; the enablement slice re-runs the same AC-id-tagged tests against the real
  ESP and owns no new coverage rows. **Not minted as an epic yet.**
