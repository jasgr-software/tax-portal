...
- **Merge policy (two lanes):** see `.orchestration/MERGE-POLICY.md`. A **docs-only** PR (every changed file is outside the "application code" scope above — `.planning/**`, `.requirements/**`, `.architecture/**`, `.implementation/**` docs, `.orchestration/**`, `.pr-review/**`, `.code-standards/**`, `.claude/**`, `CLAUDE.md`, `**/*.md`, memory) **skips the `/pr-review` panel and the code-standards Standards-review audit** and merges on green **required** CI (`lint-and-typecheck` + `security-scan`) with a plain `gh pr merge <N> --squash --delete-branch` — no `--admin`, no `enforce_admins` toggle. The Conductor may fast-lane-merge docs-only PRs autonomously. An **application-code** (or mixed) PR takes the reviewed lane: Standards-review audit → panel → fix → resolve threads → merge on green CI (still no protection toggle). Branch protection is never silently relaxed; a genuine governance gate is a user decision.
...
| Role Tag             | Agent Role           | Model · effort | Assigned Directories                                                          | Tech Stack                                                                 |
| -------------------- | -------------------- | ---------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `[io]`               | Implementation Orchestrator | Opus · high | `.implementation/AGENT.md`, `.implementation/ENGINE.md`, `.implementation/PHASES.md`, `.implementation/tasks/` | —                                                  |
| `[webapp-developer]` | Developer            | Sonnet · high | `apps/portal`, `apps/admin`, `packages/`, `prisma/`, `db/`                    | Next.js 14 (App Router), TypeScript, Clerk, Prisma (sqlserver provider), SQL Server 2022, Tailwind, shadcn/ui, Playwright, Vitest. Two front-ends: `apps/portal` (Client Portal — client-facing) and `apps/admin` (Tax Portal — accountant-facing). See ADR-006. |
| `[devops]`           | Developer            | Sonnet · high | `infra/`, `.github/workflows/`, Dockerfiles, `docker-compose*.yml`            | OCI containers (multi-stage Dockerfile), GitHub Actions, Docker + Docker Compose (local dev). Production deploy platform deferred — see ADR-007. |
| `[sdet]`             | SDET / Validator     | Sonnet · high | (reviews all directories)                                                     | —                                                                          |
| `[overwatch]`        | Overwatch            | Sonnet · medium | (reads all directories)                                                     | —                                                                          |

> **Model/effort source of truth:** each agent's own front-matter (`model:` + `effort:`) is authoritative; this table is the human overview. Model values are tier **aliases** (`opus`/`sonnet`) that auto-track the latest model in that tier — do not write specific version numbers here (a stale version-pinned label is exactly the drift that causes; this overview previously mislabeled the orchestrator's tier that way). The three PR-review lenses (`reviewer-*`, Opus · xhigh) and `pr-fixer` (Sonnet · medium) live in `.pr-review/`; `requirements` (Sonnet · medium), `code-standards` (Sonnet · medium), and the spawnable `planning`/`architecture` agents (Opus · high) are registered in `.claude/agents/`.
...
- `.architecture/` — **standalone architecture layer** (the *how*): ADRs (`decisions/ADR-*`), the C4 model (`c4/`), and strategy docs; owned by `.architecture/AGENT.md`. Spawnable as the `architecture` subagent / `/architecture` command (Opus · high) via a thin adapter in `.claude/`; the role behavior stays in `AGENT.md`.
- `.planning/` — **standalone, workflow-decoupled delivery-planning layer** (the "Product Owner"). Decomposes the requirement + architecture sources into a phased roadmap of vertically-sliced epics and tracks every acceptance criterion to sign-off. Owned by its own canonical agent, `.planning/AGENT.md` — read that to author/validate; it is self-contained and not wired into the implementation workflow. A `.planning/` epic is a canonical **producer of build briefs** for `.implementation/`. It also owns the **behavior contract**: user personas (`.planning/personas/`), targeted per-slice user flows (`.planning/flows/`, built out incrementally as epics are authored), and each epic's Given/When/Then **acceptance scenarios** (embedded in the epic — there is no standalone `.feature` tree). Key files: `.planning/ROADMAP.md` (phased plan), `.planning/COVERAGE.md` (per-AC acceptance ledger), `.planning/EPIC-NNN-*.md` (epics), `.planning/personas/` + `.planning/flows/` (behavior contract), `.planning/seed/sources.md` (declares the requirement/architecture sources — the only project-coupling point). Joins `.requirements/` (the *what*) and `.architecture/` (the *how*) as the third standalone layer (the *what-next-and-in-what-order*). Spawnable as the `planning` subagent / `/planning` command (Opus · high) via a thin adapter in `.claude/`; the role behavior stays in `AGENT.md`.
- `.pr-review/` — **standalone PR-review layer** (3-lens advisory panel + `pr-fixer`). Models: the three reviewer lenses run **Opus · xhigh**, `pr-fixer` runs **Sonnet · medium**. See `.pr-review/README.md`.
- `.orchestration/` — **the delivery Conductor** (project-coupled, main-session-driven). Drives **one ready `.planning/` roadmap slice** end-to-end — select → readiness-gate → compose build brief → implementation engine → `pr-review` → `pr-fix` → merge/finalize → coverage write-back — then **stops and reports** (re-invoke per slice). The implementation engine is a **swappable backend behind the build-brief contract**; `.implementation/` (`/io`) is the default binding, declared (with the roadmap source, review/fix, and validate capability) in `.orchestration/seed/sources.md`. Entry: `/orchestrate [EPIC-NNN]`. See `.orchestration/README.md`.
- `.code-standards/` — **standalone code-standards layer** (the project's *dos and don'ts*). Catalogues conventions as individually-keyed, rateable standards (`CS-<LANG>-NNN`, buckets `TS`/`SQL`/`GEN`/`INFRA`), each **pointing at** its owning ADR/`CLAUDE.md` authority (pointer, not copy) and carrying an enforcement-weight `rating` (`experimental`→`recommended`→`required`, terminal `deprecated`). Born at true rating; the promote/demote process is **defined** (machine proposes `by: agent`, human ratifies `by: user` — see `.code-standards/AGENT.md` § Promote/demote criteria). **Consumption is wired:** build briefs carry the applicable `CS-*`; code and tests cite the governing key in comments — `// ADR-NNN` / `// DECISION:` and `// CS-<LANG>-NNN` (CS-GEN-003) — so a reviewer can trace implementation to its authority; the SDET checks each cited standard's `verification` hook (a failing `required` standard is a rejection); and the Conductor runs a **code-standards Audit** of the opened PR before `/pr-review` (see `.orchestration/` § Standards-review). The `.pr-review/` lenses stay project-agnostic and do **not** read this layer — the audit is the separate, project-aware path. Owned by `.code-standards/AGENT.md`; spawnable as the `code-standards` subagent / `/code-standards` command (Sonnet · medium) via thin `.claude/` adapters. See `.code-standards/README.md`.
