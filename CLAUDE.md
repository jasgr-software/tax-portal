...
- **Merge policy (two lanes):** see `.orchestration/MERGE-POLICY.md`. A **docs-only** PR (every changed file is outside the "application code" scope above — `.planning/**`, `.requirements/**`, `.architecture/**`, `.implementation/**` docs, `.orchestration/**`, `.pr-review/**`, `.code-standards/**`, `.claude/**`, `CLAUDE.md`, `**/*.md`, memory) **skips the `/pr-review` panel** and merges on green **required** CI (`lint-and-typecheck` + `security-scan`) with a plain `gh pr merge <N> --squash --delete-branch` — no `--admin`, no `enforce_admins` toggle. The Conductor may fast-lane-merge docs-only PRs autonomously. An **application-code** (or mixed) PR takes the reviewed lane: panel → fix → resolve threads → merge on green CI (still no protection toggle). Branch protection is never silently relaxed; a genuine governance gate is a user decision.
