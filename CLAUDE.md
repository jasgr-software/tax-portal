...
- `.pr-review/` — **standalone, workflow-decoupled PR-review layer**. A 3-lens **independent, project-agnostic** code-review panel — `reviewer-correctness` (LEAD), `reviewer-security`, `reviewer-over-engineering` — that reviews a GitHub PR's diff + changed files on general engineering merit (the reviewers read **no** project docs — no ADRs, requirements, planning, or `CLAUDE.md` conventions — and apply no project-specific rules) and posts **one consolidated advisory review** (`event=COMMENT`, so it never blocks merge or touches branch protection); plus a **project-aware** `pr-fixer` agent that reads the panel's comments and drives the PR to green in a bounded auto-loop (running this repo's submission gate). The PR is the artifact — it runs against any open PR by number and is not coupled to the `.implementation/` task pipeline. Main-session-owned tooling (it is the orchestrator — agents can't self-spawn). Key files: `.pr-review/AGENT.md` (panel orchestration), `.pr-review/ENGINE.md` (shared rules — finding schema, dedupe, advisory verdict, comment mechanics), `.pr-review/agents/*` (the 3 lenses + the fixer), `.pr-review/seed/sources.md` (the fixer's project-coupling point — gate/CI commands; the reviewers need nothing from it). Run via `/pr-review <PR#>` and `/pr-fix <PR#>`. Models: reviewers Opus, fixer Sonnet. Complements (does not replace) the `/code-review` + `/security-review` skills and the `.implementation/` SDET review.
